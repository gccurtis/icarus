...
Regions compactly style large blank ranges and do not materialize Cells.
...
Ordinary Range and span endpoints must exist in the same Sheet. Range
membership is the inclusive rectangular interval between its two stable
endpoints in current axis order, regardless of which endpoint currently sorts
first. An axis move may therefore reverse their visual order without rewriting
their stored authored roles. Inserting an axis between them expands the range
predictably.

A `CellSpan` is stricter: its Cell anchor must remain the visual top-left and
its end must remain at-or-after the anchor. An axis move that would invert a
merged span is rejected unless the same batch first unmerges or explicitly
rewrites it. Operations that delete an ordinary range/span endpoint must
explicitly rewrite, remove, or reject that owner; silently retargeting to a
neighboring axis is invalid. Formula manifests use the broken-target contract
below instead of invalidating the Workbook.
...
interface FormulaCellAddress {
  sheetId: SheetId;
  rowId: RowId;
  columnId: ColumnId;
}

interface FormulaRangeAddress {
  sheetId: SheetId;
  startRowId: RowId;
  startColumnId: ColumnId;
  endRowId: RowId;
...
interface MissingFormulaIdentity {
  kind: "sheet" | "row" | "column";
  id: string;
}

type SpreadsheetFormulaCellTarget =
  | { state: "resolved"; ref: StableCellRef }
  | {
      state: "broken";
      priorAddress: FormulaCellAddress;
      missing: MissingFormulaIdentity[];
    };

type SpreadsheetFormulaRangeTarget =
  | { state: "resolved"; ref: StableRangeRef }
  | {
      state: "broken";
      priorAddress: FormulaRangeAddress;
      missing: MissingFormulaIdentity[];
    };
...
interface SpreadsheetCellFormulaBinding {
  ...
  target: SpreadsheetFormulaCellTarget;
...
interface SpreadsheetRangeFormulaBinding {
  ...
  alias: string;
  authoredSpan: FormulaAuthoredSpan;
  target: SpreadsheetFormulaRangeTarget;
...
Axis deletion canonicalizes each affected grid binding from `resolved` to
`broken`, retaining the prior stable IDs and an exact missing-identity list,
and recomputes the owning source digest in the same ChangeSet. Evaluation then
returns `broken_reference`; it never falls back to the new occupant of an A1
position or to a name. Undo restores the original IDs and resolved target
through the exact inverse. Ordinary `StableCellRef`/`StableRangeRef` values
elsewhere continue to require live endpoints.
...
targets. Deleting a referenced axis yields the typed broken target above. The
stored relative/absolute axis modes preserve the user's `$` intent for display
and a future copy/fill authoring command. Representation v1 has no copy/fill
command; callers author each destination Formula through normal admission.
...
Custom validation and Formula Conditional Formatting must evaluate to Formula
logic. Validation accepts only `true`; `false`, null, a non-logic result, or an
evaluation diagnostic rejects the candidate and returns the configured message
plus a typed diagnostic. Conditional Formatting applies its Style only for
`true`; `false` is a normal non-match, while null/non-logic/error is a reported
non-match diagnostic. Value conditions use Formula's strict kind comparison
and never coerce text, number, logic, or null implicitly.
...
interface RangeProjection {
  ...
  /** Absent when invalid shape or axis capacity prevents a valid endpoint. */
...
interface SpreadsheetChartBase {
  title?: RichContent;
  showLegend: boolean;
...
type SpreadsheetChart =
  | CartesianSpreadsheetChart
  | PieSpreadsheetChart
  | ScatterSpreadsheetChart;

type SpreadsheetChartAxisKind = "category" | "value" | "x" | "y";

interface CartesianSpreadsheetChart extends SpreadsheetChartBase {
  kind: "column" | "bar" | "line" | "area";
  axes?: {
...
interface PieSpreadsheetChart extends SpreadsheetChartBase {
  kind: "pie";
  /** A Pie has exactly one series and no axes. */
  series: PieChartSeries;
}

interface ScatterSpreadsheetChart extends SpreadsheetChartBase {
  kind: "scatter";
  axes?: {
    x?: SpreadsheetChartAxis;
    y?: SpreadsheetChartAxis;
  };
...
interface ScatterChartSeries {
  id: string;
  name?: RichContent;
  xValues: StableRangeRef;
  yValues: StableRangeRef;
  color?: WorkbookColorValue;
}
...
Chart title, axis titles, and series names are Rich Content and use the same
project-only Formula-atom boundary described above. Cartesian series require
equal-length value/category ranges when categories are present. Pie requires
one equal-length category/value pair and cannot carry axes. Scatter requires
equal-length numeric X/Y ranges; its X and Y axes are distinct from Cartesian
category/value axes. Missing, pending, error, or nonnumeric value coordinates
produce typed Chart diagnostics rather than coercion. Images store an immutable
General Files snapshot reference, not bytes or a mutable file head. Alternative
text is a plain accessibility string; a decorative image requires an empty
`alt`.
