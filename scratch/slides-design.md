...
Text-bearing surfaces (a Text element's body, a table cell, slide notes) hold a
TextSource: authored Rich Content, or a reference to a Derived Output.
```
...
interface Slide {
  ...
  notes: SlideTextSource;            // authored, or generated by a prompt
  elements: Record<SlideElementId, SlideElement>;
}
...
/** The three surfaces that hold one. */
//  TextElement.body   TableCell.body   Slide.notes
```
...
kind and no new machinery — it is a cell whose body happens to be a
`prompt` source. The same holds for speaker notes, which is a genuinely useful
thing to generate.
...
and keeping them plain keeps the union to the three surfaces that earn it.
...
```ts
type PromptSite =
  | { kind: "text-element"; slideId: SlideId; elementId: SlideElementId }
  | { kind: "table-cell";   slideId: SlideId; elementId: SlideElementId; cellId: TableCellId }
...
type PromptCreateTarget =
  ...
  | { kind: "new-text-element"; slideId: SlideId;
