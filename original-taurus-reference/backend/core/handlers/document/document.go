...
func (h Handlers) List(ctx access.Context, _ endpoint.Request) endpoint.Response {
	...
	if err != nil {
		return endpoint.Fail(http.StatusInternalServerError, "could not list documents", err)
	}
	if h.canAccess != nil {
...
func (h Handlers) Create(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	if strings.TrimSpace(in.FromTemplateID) != "" {
		...
		if err != nil {
			return endpoint.Fail(http.StatusInternalServerError, "could not create from template", err)
		}
	...
	if err != nil {
		return endpoint.Fail(http.StatusInternalServerError, "could not create document", err)
	}
	return endpoint.Response{Status: http.StatusCreated, Body: d}
}
...
func (h Handlers) Templates(ctx access.Context, _ endpoint.Request) endpoint.Response {
	...
	if err != nil {
		return endpoint.Fail(http.StatusInternalServerError, "could not list templates", err)
	}
	if templates == nil {
...
func (h Handlers) Get(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	if err != nil {
		return endpoint.Fail(http.StatusInternalServerError, "could not get document", err)
	}
...
func (h Handlers) Rename(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case errors.Is(err, doc.ErrNotFound):
		return errResp(http.StatusNotFound, "document not found")
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not rename document", err)
	}
	return endpoint.Response{Status: http.StatusOK, Body: d}
}
...
func (h Handlers) Delete(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case errors.Is(err, doc.ErrNotFound):
		return errResp(http.StatusNotFound, "document not found")
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not trash document", err)
	}
...
func (h Handlers) Restore(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not restore document", err)
	}
...
func (h Handlers) Purge(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case errors.Is(err, doc.ErrNotFound):
		return errResp(http.StatusNotFound, "document not found or not in trash")
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not purge document", err)
	}
...
func (h Handlers) Duplicate(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case errors.Is(err, doc.ErrNotFound):
		return errResp(http.StatusNotFound, "document not found")
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not duplicate document", err)
	}
	return endpoint.Response{Status: http.StatusCreated, Body: d}
}
...
func (h Handlers) Diff(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not diff document", err)
	}
...
func (h Handlers) CreateAnchor(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not create anchor", err)
	}
...
func (h Handlers) ListAnchors(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case errors.Is(err, doc.ErrNotFound):
		return errResp(http.StatusNotFound, "document not found")
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not list anchors", err)
	}
...
func (h Handlers) DeleteAnchor(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case errors.Is(err, doc.ErrNotFound):
		return errResp(http.StatusNotFound, "document not found")
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not delete anchor", err)
	}
...
func (h Handlers) ValidateAnchor(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not validate anchor", err)
	}
...
func (h Handlers) AppendChanges(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not apply changes", err)
	}
...
func (h Handlers) History(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not list document history", err)
	}
	var nextCursor any
	if page.NextCursor != "" {
...
func (h Handlers) GetChangeSet(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not get document change set", err)
	}
...
func (h Handlers) Undo(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not undo change", err)
	}
	return endpoint.Response{Status: http.StatusCreated, Body: cs}
}
...
func (h Handlers) Redo(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	case err != nil:
		return endpoint.Fail(http.StatusInternalServerError, "could not redo change", err)
	}
	return endpoint.Response{Status: http.StatusCreated, Body: cs}
}
...
func invalidStyleResp(err *doc.StyleValidationError) endpoint.Response {
	return endpoint.Response{
		...
		// The typed cause contains code/field/reason but never the rejected value.
		Err: err,
	}
}
...
func (h Handlers) RevisionHints(ctx access.Context, _ endpoint.Request) endpoint.Response {
	...
	if err != nil {
		return endpoint.Fail(http.StatusInternalServerError, "could not get revision hints", err)
	}
