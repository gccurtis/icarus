...
type WorkflowOptions struct {
	...
	Enqueuer job.Enqueuer
	// ResourceTools, when set, provides caller-aware exact Resource listing and
	// reading to durable Plan and Action runs as well as Ask turns.
...
func NewWorkflows(opts WorkflowOptions) (*Workflows, error) {
	...
	return &Workflows{tasks: opts.Tasks, personas: opts.Personas, documents: opts.Documents, runner: runner, notifier: opts.Notifier, authorizer: opts.Authorizer, enqueuer: opts.Enqueuer, resTools: opts.ResourceTools}, nil
}
...
func (w *Workflows) RunJob(ctx context.Context, raw json.RawMessage) error {
	...
	// Attribute every provider call this run makes — planning, the tool loop, any
	// corrective re-ask — to the task, so one run's true spend is a single grep
	// rather than a correlation by timestamp across concurrent jobs.
...
func (w *Workflows) run(ctx context.Context, scope Scope, task Task, runID string) error {
	...
	if w.resTools != nil {
		bindings = append(bindings, w.resTools.ListTool(ResourceScope{ProjectID: scope.ProjectID, CallerID: scope.CallerID}), w.resTools.ReadTool(ResourceScope{ProjectID: scope.ProjectID, CallerID: scope.CallerID}))
	}
