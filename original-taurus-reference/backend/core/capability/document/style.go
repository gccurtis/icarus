...
type StyleOverrides struct {
	...
	// CustomTypography is a custom typography escape hatch. Values are admitted
	// through the canonical font and color grammar before they are stored.
...
// Custom typography field length bounds are part of the canonical grammar.
const (
...
// one of the admitted CSS units round-trips unchanged. Foreground (fg) and
...
// validateCustomTypography applies the shared font/color contract. A nil
// payload is valid because it clears custom typography.
func validateCustomTypography(custom *CustomTypography) error {
	if custom == nil {
		return nil
	}
	return ValidateCustomTypography(*custom)
}
