---
title: "Α-018 — Resource Replica Hydration, Canonical State & Cache"
notion_page_id: "3adb6410e5028118a42ae8adeb21bb3f"
notion_url: "https://app.notion.com/3adb6410e5028118a42ae8adeb21bb3f"
...
# Α-018 — Resource Replica Hydration, Canonical State & Cache

> Source mirror. Preserve this page as planning evidence; use the agent-facing execution packet when one exists.

> **Status:** Queued
> **Wave:** Wave 2 — User Cell and Project Subcell runtime
> **Outcome:** Build view-independent resource replicas with confirmed snapshots, canonical revisions, derived projections, explicit load status, bounded cache, invalidation, and acquisition by stable resource identity.
## Why this packet exists
Build view-independent resource replicas with confirmed snapshots, canonical revisions, derived projections, explicit load status, bounded cache, invalidation, and acquisition by stable resource identity. This packet is bounded so an implementation agent can complete it without inventing adjacent architecture. It is not complete until the old behavior is either migrated with parity evidence or explicitly removed.
## Current Alpha evidence to inspect
- Document runtime registry and sync model
- resource API/store coupling
- localStorage Workspace boot state
...
- Define confirmed, optimistic, pending, stale, refused, failed, and disposed states.
- Keep wire DTOs and editor-engine state out of generic replica contracts.
- Associate cache entries with Project, resource, revision, schema, User/session generation, and acknowledgement.
- Bound memory and invalidate on revocation, mismatch, or unsupported migration.
### Specific completion gate
Cache accelerates startup but is never authority; stale startup visibly reconciles to Omega, and one replica safely supports multiple mounted views.
