---
title: "Α-016 — Per-User Project Subcell Registry & Lifecycle"
notion_page_id: "3adb6410e502815ca220dc2df0e22d7e"
notion_url: "https://app.notion.com/3adb6410e502815ca220dc2df0e22d7e"
...
# Α-016 — Per-User Project Subcell Registry & Lifecycle

> Source mirror. Preserve this page as planning evidence; use the agent-facing execution packet when one exists.

> **Status:** Queued
> **Wave:** Wave 2 — User Cell and Project Subcell runtime
> **Outcome:** Implement the frontend Project runtime registry corresponding to one User Cell's logical `(UserID, ProjectID)` backend Project Subcell, with leases, reuse, suspension, eviction, revocation, and disposal.
## Why this packet exists
Implement the frontend Project runtime registry corresponding to one User Cell's logical `(UserID, ProjectID)` backend Project Subcell, with leases, reuse, suspension, eviction, revocation, and disposal. This packet is bounded so an implementation agent can complete it without inventing adjacent architecture. It is not complete until the old behavior is either migrated with parity evidence or explicitly removed.
## Current Alpha evidence to inspect
- services/project-runtime.ts beside direct feature loads
- route-owned project state
- global project/session stores
...
- Key runtimes by User Cell identity, ProjectID, and relevant generation.
- Distinguish browser tabs/windows from Workspace tabs and document the no-SharedWorker boundary.
- Reference-count Project, replica, and view leases separately.
- Dispose immediately on sign-out/revocation and only evict idle runtimes after pending work is recoverable.
### Specific completion gate
One browser application instance reuses one runtime for the same User/Project across Workspace views; different Users and independent browser windows have independent frontend memory and converge through Omega.
