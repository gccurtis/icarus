---
title: "Α-019 — Optimistic Operation Queue, Idempotency & Acknowledgement"
notion_page_id: "3adb6410e50281269f00ed95f9ab08b4"
notion_url: "https://app.notion.com/3adb6410e50281269f00ed95f9ab08b4"
...
# Α-019 — Optimistic Operation Queue, Idempotency & Acknowledgement

> Source mirror. Preserve this page as planning evidence; use the agent-facing execution packet when one exists.

> **Status:** Queued
> **Wave:** Wave 2 — User Cell and Project Subcell runtime
> **Outcome:** Implement per-resource optimistic queues with base revision, durable SubmissionID/idempotency key, local prediction, bounded ordering, acknowledgement matching, and presentable pending state.
## Why this packet exists
Implement per-resource optimistic queues with base revision, durable SubmissionID/idempotency key, local prediction, bounded ordering, acknowledgement matching, and presentable pending state. This packet is bounded so an implementation agent can complete it without inventing adjacent architecture. It is not complete until the old behavior is either migrated with parity evidence or explicitly removed.
## Current Alpha evidence to inspect
- Document optimistic diff queue
- opaque Workspace PUT
- local Slides store commands
...
- Use one durable acceptance identity across retries and a separate trace RequestID per attempt.
- Reject unsupported predictions before applying optimistic state.
- Keep accepted operations in history; undo submits a new semantic command.
- Set queue budgets and typed recovery when replay/rebase is unsafe.
### Specific completion gate
Double click, duplicate acknowledgement, delayed response, view unmount, retry, and duplicate change descriptors cannot duplicate, silently lose, or misattribute a mutation.
