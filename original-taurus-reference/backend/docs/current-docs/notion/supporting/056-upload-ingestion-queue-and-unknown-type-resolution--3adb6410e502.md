---
title: "Α-056 — Upload, Ingestion Queue & Unknown-Type Resolution"
notion_page_id: "3adb6410e50281c482a9c67a509bb54f"
notion_url: "https://app.notion.com/3adb6410e50281c482a9c67a509bb54f"
...
# Α-056 — Upload, Ingestion Queue & Unknown-Type Resolution

> Source mirror. Preserve this page as planning evidence; use the agent-facing execution packet when one exists.

> **Status:** Queued
> **Wave:** Wave 6 — AI, ingestion and interchange
> **Outcome:** Implement file picker/drop, byte/size validation, upload progress, ingestion jobs, Text/Structured Data/Media classification status, OCR choice/result, retry/cancel, typed failures, and unknown-type user resolution.
## Why this packet exists
Implement file picker/drop, byte/size validation, upload progress, ingestion jobs, Text/Structured Data/Media classification status, OCR choice/result, retry/cancel, typed failures, and unknown-type user resolution. This packet is bounded so an implementation agent can complete it without inventing adjacent architecture. It is not complete until the old behavior is either migrated with parity evidence or explicitly removed.
## Current Alpha evidence to inspect
- ImportDialog and transfer helpers
- no multi-lattice ingestion queue UI
- generic file-kind affordances
...
- Present mutually exclusive Text, Structured Data, and Media projections while allowing one source to yield separate Media descriptor and OCR-derived Text.
- Support unknown-type choices with typed server revalidation and recovery.
- Show progress, queue position/stage, warnings, failure diagnostics, retry, cancel, and result opening.
- Defer audio ingestion/recording and legacy XLS unless a later backend contract adds them.
### Specific completion gate
Extension or user classification never bypasses server byte validation; job state survives navigation/reload where supported; text, CSV/XLSX, and PNG/JPEG/WebP paths report truthful cost/status.
