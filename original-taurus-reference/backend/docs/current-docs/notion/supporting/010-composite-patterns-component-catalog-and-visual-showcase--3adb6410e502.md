---
title: "Α-010 — Composite Patterns, Component Catalog & Visual Showcase"
notion_page_id: "3adb6410e502818d9a73f95b5271fea6"
notion_url: "https://app.notion.com/3adb6410e502818d9a73f95b5271fea6"
...
# Α-010 — Composite Patterns, Component Catalog & Visual Showcase

> Source mirror. Preserve this page as planning evidence; use the agent-facing execution packet when one exists.

> **Status:** Queued
> **Wave:** Wave 1 — Component and visual substrate
> **Outcome:** Complete shared composite patterns and turn /components into the checked source for every small component, state, responsive behavior, accessibility contract, and visual baseline.
## Why this packet exists
Complete shared composite patterns and turn /components into the checked source for every small component, state, responsive behavior, accessibility contract, and visual baseline. This packet is bounded so an implementation agent can complete it without inventing adjacent architecture. It is not complete until the old behavior is either migrated with parity evidence or explicitly removed.
## Current Alpha evidence to inspect
- 51 exported primitives and Taurus surfaces
- `/components gallery`
- resource/library list implementations
...
- Add virtualized collection/table contracts without selecting a proprietary dependency.
- Catalogue loading, empty, failure, permission, offline, and destructive states.
- Provide density, narrow-width, zoom, forced-color, reduced-motion, and keyboard fixtures.
- Remove duplicate template/resource collection patterns once parity is proven.
### Specific completion gate
Every smaller component in the completion census has a catalog owner and fixture; composite patterns cover property rows, panels, search, tree/list/table, tab strip, command menu, pickers, and long collections.
## Target boundary and contract
```typescript
interface ComponentBoundary<Props, Event> {
  props: Readonly<Props>;
  states: readonly SemanticState[];
  emit(event: Event): void;
  // No transport, authorization, or canonical business state.
}
```
...
This packet includes production Svelte/HTML and CSS-token implementation—not only TypeScript interfaces. Use native semantics first, centralized tokens only, stable component states, and complete catalog fixtures. Styling values come only from the centralized visual authorities. Any intentional visual change must have before/after evidence and an updated component or screen fixture.
