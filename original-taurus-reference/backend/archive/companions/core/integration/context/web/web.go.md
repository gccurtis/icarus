...
Web-search adapter implementing agent.WebRetriever against a JSON search endpoint. Every request is bounded — query length, result count, a response-body cap, and an HTTPS-only endpoint — so one lookup can neither run away nor reach a private address over plain HTTP. See repo conventions (AGENTS.md).
