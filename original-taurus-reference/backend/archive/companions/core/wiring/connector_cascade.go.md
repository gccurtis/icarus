# connector_cascade.go

The concrete `connector.Cascader` (Slice G): when a connector sync changes a
source, refresh every prompt block that depends on it. It maps the synced
connector to a `document.ScopeOrigin`, asks the reference graph
(`Documents.DependentPrompts`, Slice F) who depends on it, and enqueues a
`document.resolve` job (mode `reload`) per dependent. Refresh is **best-effort**:
a failure is logged, never propagated back into the sync. Resolution is already
authored by the system actor and logged in Activity, so attribution needs no code
here. Living in wiring keeps the connector capability independent of `document`
and the job queue. See repo conventions (AGENTS.md).
...
The payload shape matches the transport's own resolve enqueue, so a cascade-
enqueued job and a user-triggered resolve run through the identical handler.
