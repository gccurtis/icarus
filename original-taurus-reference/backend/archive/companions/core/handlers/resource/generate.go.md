...
"Create with AI": the ResourceGenerator port plus the Generate handler that creates a resource through the canonical family owner and kicks off an agent Action to populate it, returning the resource and the populating task id. See repo conventions (AGENTS.md).
