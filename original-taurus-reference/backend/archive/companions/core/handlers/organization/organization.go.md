...
Above-Project organization endpoints: create an org (caller becomes owner), list the caller's own, rename, and manage memberships (list/add/set-role/remove). Gated on a signed-in user but not scoped to a selected Project. Maps capability errors to HTTP statuses (403 forbidden, 404 not found, 409 last-owner). See repo conventions (AGENTS.md).
