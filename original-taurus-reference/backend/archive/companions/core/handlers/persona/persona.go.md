...
func (h Handlers) Tasks(ctx access.Context, req endpoint.Request) endpoint.Response {
	...
	if err != nil {
		return errResp(http.StatusInternalServerError, "could not list persona tasks")
	}
	if tasks == nil {
		tasks = []agent.Task{}
	}
...
func personaErr(err error) endpoint.Response {
	...
	default:
		return errResp(http.StatusInternalServerError, "could not process persona request")
	}
}
...
Its 2 failure responses (`could not list persona tasks`, and the rest)
