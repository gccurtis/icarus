...
Access is the authentication and authorization subsystem: user registration,
login, password management, public identity projection, session management, and
project-scoped access control.

It also owns the *names and shapes of the two cookies* the transport layer
carries, because both sides have to agree on them: `SessionCookieName`
(`to_session`), the opaque session id, and `CSRFCookieName` (`to_csrf`), the
double-submit CSRF token. The two are deliberately asymmetric. The session
cookie is HttpOnly — it is the credential, and scripts must never read it. The
CSRF cookie is not, because the browser client has to read it and echo the value
back in a request header; that echo is the whole mechanism, and the token grants
nothing on its own. `NewCSRFToken` mints one, reusing the same 32-byte
`crypto/rand` hex value behind session and invite tokens, so there is one notion
of "unguessable token" in this package rather than three. Access defines the
token and its name; the transport gate decides *when* to issue it and the
transport middleware decides *what it means* if it does not match.
