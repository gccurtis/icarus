...
func (w *Workflows) documentGetTool(scope Scope, task Task) intelligence.ToolBinding {
	return intelligence.ToolBinding{Definition: intelligence.ToolDefinition{
		...
		Description:  "Read a document ... as an ordered list of blocks ... Call this before editing so block ids and content are current. An empty blocks array means the document exists and has no content yet — append to it as normal. A document that does not exist returns an error instead.",
...
### `stripBlockMarker` — the kind already said that

A model writing markdown writes the block marker too: it sends kind
`heading_1` *and* text `# Title`. `ParseBlockMarkdown` only handles inline
spans, so without intervention the marker survives into the atom and the
heading reads `# Title` — rendered once as structure and once as content. This
was caught by the live `agents` suite, whose document assertion expected the
bare title.

`stripBlockMarker` removes only the marker the declared kind already expresses:
any depth of `#` run followed by whitespace for a heading (the kind, not the
hash count, decides the level), and a leading `>` for a quote. A paragraph
about `#hashtags` keeps its text, and `#nospace` is left alone because a marker
is only a marker when whitespace follows it.

It takes the **model-facing kind**, not the stored sub-kind, because the two are
not interchangeable here: `quote` is stored as body text, so by the time you
hold the sub-kind, the fact that a quote marker was meant is gone.

### `document.get` — why an empty document says so out loud

A `rows: []` document and a missing document look identical to an agent that
only reads the block list: both come back with nothing in them. A live run made
the consequence concrete — the agent was told to write into a freshly created
document, read it, found no blocks, concluded the document did not exist, and
reported the task blocked instead of writing the content it had been asked for.

The tool's answer already distinguished the two cases — a document that does not
exist returns an error, not an empty list — but nothing said so, and a model
cannot infer a contract it was never given. The description now states both
readings explicitly: empty means present and unwritten, so append as normal;
absent raises an error instead.

This is a description change, not a behaviour change. Tool descriptions are part
of the prompt, and a correct tool with an under-specified description fails in
exactly the way a buggy one does.
