...
// hardToolLimits are the fixed ceilings for this first implementation. A
...
### Why the ceiling is 64 rounds

Sized for **document authoring**, which is a headline use case rather than an
edge case. An agent writing a structured document appends roughly one block per
round, so the original 16-round ceiling bought a title, three sections and
little else — a live run asking for a 400-word story exhausted it and failed the
task outright. 64 rounds (256 calls, 512k tokens) leaves room for a real
document while still bounding a runaway loop. These are hard ceilings: a
deployment can pass lower values per request, but no caller or model can raise
them.
