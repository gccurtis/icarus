...
Ephemeral toast notifications: a bounded, in-memory, per-user, Project-scoped queue. Push assigns a toast's id/createdAt, clamps its text, and defaults an unknown level to info; Drain destructively returns a user's toasts for one Project (delivered exactly once). A process restart may drop undelivered toasts by design — nothing treats a toast as a source of truth. See repo conventions (AGENTS.md).
