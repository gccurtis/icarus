...
`value.go` defines Formula's immutable runtime data model. Scalars remain strongly
typed—null, exact rational number, valid UTF-8 text, and logic—while list, record, and
table values share one rectangular `Table` carrier: a list is one field by many rows, a
record is many fields by one row, and a full table is many fields by many rows.

Construction validates field identity, UTF-8, rectangular rows, and numeric size before
expensive arbitrary-precision parsing. Public structured accessors return deep copies,
deterministic string and JSON forms preserve type and shape, and strict decoding rejects
malformed, noncanonical, or ambiguous values.
...
This file belongs to the pure `formula` package and supplies the value layer consumed by
...
The imports support strict JSON decoding, descriptive validation errors, bounded
arbitrary-precision rationals, deterministic text rendering, and UTF-8 checks. `bytes`
and `io.EOF` let decoding require exactly one JSON object after the kind-specific field
pass.
...
`Kind` is the closed runtime type tag. The constants cover four scalar kinds and the
three table-shaped structured kinds, letting operators and functions enforce types
without coercion.

### The KindNull constants
...
This block defines the constants beginning with KindNull for this part of the value model.
`KindFunction` tags a user-defined `FUNCTION`/`LAMBDA` value; its payload lives in the
unexported `fn *functionValue` field below rather than the shared `Table` carrier.
...
`Shape` describes every value in field-by-row terms. Scalars report 1×1; the same
dimensions naturally distinguish lists, records, and full tables.

### Rectangular table carrier
...
`Table` stores ordered field names, rectangular rows, and a derived name-to-column
index. Its fields remain private so callers cannot bypass validation or mutate values
...
`NewTable` is the public constructor and `newTable` adds source-span context for
internal callers. Construction rejects empty, invalid-UTF-8, or duplicate fields and
ragged rows, copies both axes, clones every cell, and builds the lookup index once.

### The newTable operation
...
`newTable` builds and validates a table value, and is the single choke point every
record and table flows through. Beyond the empty, UTF-8, and duplicate checks, it
now rejects any field name that is not identifier-shaped (`validFieldName`), so a
name a dot cannot spell — one with a space, say — cannot enter the value model even
through the Go API. Rows are shape-checked against the field count and deep-copied.
...
The public table accessors return copies of field names, rows, and columns, while
`Shape` reports dimensions. No accessor exposes the carrier's backing slices or nested
values.

### The Table Rows operation
...
This block implements Table Rows as one bounded part of the Formula value model.

### The Table Shape operation
...
This block implements Table Shape as one bounded part of the Formula value model.

### The Table Field operation
...
This block implements Table Field as one bounded part of the Formula value model.

### Internal table access and cloning
...
These package-private helpers copy a row, copy a named cell, or rebuild an entire table.
They let evaluator code work efficiently with validated data while maintaining
immutability at every returned boundary.

### The Table cell operation
...
This block implements Table cell as one bounded part of the Formula value model.

### The Table clone operation
...
This block implements Table clone as one bounded part of the Formula value model.

### Tagged immutable value
...
`Value` is a compact tagged union. Only the payload selected by `kind` is meaningful;
structured kinds point at the common table carrier, exact numbers use
arbitrary-precision rationals, and a function value points at its `functionValue`
payload (parameters, body, source; see `function.go`).
...
`NullValue` constructs Formula's explicit null scalar. Null remains distinct from an
invalid zero-value `Value` and from every empty structured value.

### Bounded exact-number construction
...
`NumberValue` applies the production number-bit ceiling, while `parseNumberValue`
accepts a caller-selected tighter ceiling and source span. `preflightNumber` validates
decimal, exponent, or rational spellings and rejects obviously oversized magnitudes
before `big.Rat` can allocate them; the parsed numerator and denominator receive a final
exact bit check.

### The parseNumberValue operation
...
This block implements parseNumberValue as one bounded part of the Formula value model.

### The preflightNumber operation
...
This block implements preflightNumber as one bounded part of the Formula value model.

### Numeric spelling bounds
...
These helpers validate signed and unsigned decimal components and conservatively
translate a bit ceiling into a maximum decimal-digit count. Overflow-safe arithmetic
keeps the preflight itself bounded even for extreme configured inputs.

### The signedDecimalDigits operation
...
This block implements signedDecimalDigits as one bounded part of the Formula value model.

### The maxDecimalDigits operation
...
This block implements maxDecimalDigits as one bounded part of the Formula value model.

### Exact-number copy helpers
...
Internal rational and integer constructors always allocate a fresh `big.Rat`. That copy
boundary prevents mutable arbitrary-precision values from being shared across otherwise
immutable Formula values.

### The integerValue operation
...
This block implements integerValue as one bounded part of the Formula value model.

### Text and logic constructors
...
`TextValue` accepts only valid UTF-8 and returns a structured error otherwise,
preserving exact JSON round trips. The logic constructor preserves its scalar type
directly; Formula applies no implicit truthiness.

### The LogicValue operation
...
This block implements LogicValue as one bounded part of the Formula value model.
...
Lists become the reserved single `value` column, records become one ordered row, and
tables accept arbitrary rectangular rows. All three routes pass through validated
`Table` construction and `structuredValue` clones the resulting carrier before storing
it.

### The RecordValue operation
...
This block implements RecordValue as one bounded part of the Formula value model.

### The TableValue operation
...
This block implements TableValue as one bounded part of the Formula value model.

### The structuredValue operation
...
This block implements structuredValue as one bounded part of the Formula value model.

### Kind and shape accessors
...
`Kind` exposes the stable type tag, and `Shape` projects either the structured carrier's
dimensions or the conceptual 1×1 scalar shape.

### The Value Shape operation
...
This block implements Value Shape as one bounded part of the Formula value model.
...
Number, canonical number string, text, and logic accessors are type-checked. The numeric
accessor returns a new rational, preserving immutability even though `big.Rat` itself is
mutable.

### The Value NumberString operation
...
This block implements Value NumberString as one bounded part of the Formula value model.

### The Value Text operation
...
This block implements Value Text as one bounded part of the Formula value model.

### The Value Logic operation
...
This block implements Value Logic as one bounded part of the Formula value model.
...
`Table`, `Items`, `Fields`, and `Field` expose structured values in their natural forms
while returning copies. Record field access returns a scalar value; table field access
returns the selected column as a list.
...
This block implements Value Items as one bounded part of the Formula value model.

### The Value Fields operation
...
This block implements Value Fields as one bounded part of the Formula value model.

### The Value Field operation
...
This block implements Value Field as one bounded part of the Formula value model.
...
`Value.clone` duplicates mutable number and table payloads and safely copies scalar
values directly. `cloneValues` applies that rule to slices, forming the common
immutability primitive used throughout the model.

### The cloneValues operation
...
This block implements cloneValues as one bounded part of the Formula value model.
...
`Equal` first requires identical kinds, then compares exact scalar payloads or
recursively compares structured field order, dimensions, and cells. Field and row order
are intentionally semantic rather than normalized away. Two function values are equal
only when their parameter lists and exact source text match; the closures they may
carry are never compared, since equality is meant to be a syntactic check.
...
`String` renders every value in a stable human-readable notation: quoted text, canonical
exact numbers, bracketed lists, braced records, and record-oriented tables. Invalid
internal values are made visible rather than guessed at. A function value renders as its
exact defining source text (`FUNCTION(x, x * 2)`), not a synthetic descriptor.

### Canonical rational formatting
...
`formatRat` emits a decimal only when the reduced denominator contains no prime factors
beyond two and five; repeating values remain reduced fractions. Powers of two are
counted directly and `factorCount` finds powers of five with a bounded binary search,
avoiding a division loop proportional to the decimal length.

### The factorCount operation
...
This block implements factorCount as one bounded part of the Formula value model.
...
JSON output always carries kind and shape, stores exact numbers as strings, and includes
only the payload appropriate to the type. Structured rows recursively use the same typed
value representation. A function value is never serialized to a form that can round-trip:
it emits a display-only descriptor (`kind`, `params`, `source`) with no `body`, and
`UnmarshalJSON` refuses to decode `kind: function` at all (see below).
...
Decoding first validates UTF-8 and the kind-specific allowed field set, then rejects
unknown or inapplicable payloads, trailing JSON values, missing or wrongly typed data,
invalid table shapes, noncanonical list/record carriers, unknown kinds, and shape
metadata that disagrees with the payload. Assignment occurs only after the entire value
passes validation. `kind: function` is rejected up front — function values are never
decoded, only ever produced by evaluation.
