...
Reference extraction: the ReferenceIndexer port plus the walk that turns a resolved document's inline link marks into the outgoing links the reference graph records after every create and change. Best-effort, so a graph failure never fails the edit. See repo conventions (AGENTS.md).
