...
Current state companion for `history.go`. This file mirrors the source exactly so the documented view cannot drift from the implementation.

History inspection: bounded summaries, affected-object metadata, cursor-based pagination, ChangeSet retrieval, eligibility for undo/redo, and operation summarization for all typed ChangeOps.

## Code breakdown

### Complete source

### Project scoping of the document loads (DEF-1)

`History` and `ChangeSet` now pass their own `projectID` into
`store.DocumentByID`, which filters on it, so a document in another project is
`ErrNotFound` at the store. Both methods still compare `doc.ProjectID` against
`projectID` afterwards; that comparison is **deliberately redundant** with the
store's filter and stays — two independent layers, neither load-bearing alone.
...
func (d *Documents) ChangeSet(projectID, documentID, changeSetID string) (ChangeSet, error) {
	...
	return d.store.ChangeSetByID(documentID, changeSetID)
}
