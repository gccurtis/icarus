...
Current state companion for `editing.go`. This file mirrors the source exactly so the documented view cannot drift from the implementation.

Fine-grained editing operations: splice atom text with digest-guarded preconditions, move row/block/atom with source position checks, update mark, split/join text blocks, and their inverses. Block movement normalizes row tracks.
