...
Current state companion for `layout.go`. This file mirrors the source exactly so the documented view cannot drift from the implementation.

Layout vocabulary: LayoutUnit, alignments, PageLayout, LayoutRules (with CharWidth for font-map width computation), RowStyle (PageBreak/KeepWithNext), BlockStyle (with LineHeight), derived Page projection, validation, normalization, and Paginate with text-wrapping, track-aware block widths, headers/footers, and flow controls.
