...
In-memory implementation of the document Store interface for tests.

## Code breakdown

### Project scoping of `DocumentByID` (DEF-1)

`DocumentByID` takes the project id as its first parameter, mirroring the SQLite
store's `WHERE id = ? AND project_id = ?`. The map lookup by id is unchanged; the
stored document's `ProjectID` is compared against the argument and a mismatch is
`ErrNotFound`, indistinguishable from a missing document. Keeping the in-memory
store honest here is what lets capability tests prove the boundary without a
database.
