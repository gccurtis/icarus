...
Block-level markdown render/parse: a small inline subset (bold, italic, code, strike, link) so the agent reads and writes prose without byte offsets. Reused by export/import. See repo conventions (AGENTS.md).
