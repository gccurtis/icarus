...
The typography cascade resolver. An atom span's effective typography resolves per property (font family, size, foreground color, background) from the first level that sets it: inline mark, block override, sub-kind default, document default, then the built-in sub-kind typography. Per-property, not wholesale. See repo conventions (AGENTS.md).
