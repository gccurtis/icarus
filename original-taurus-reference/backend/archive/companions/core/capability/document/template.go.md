...
Document templates: the optional Base.Template descriptor (context variables with free-text bindings), the set_template / set_context_variable change ops' helpers, Templates() / CreateFromTemplate(), and the bound-context hook that supplies a document's variables to prompt synthesis. Templates change context only. See repo conventions (AGENTS.md).
