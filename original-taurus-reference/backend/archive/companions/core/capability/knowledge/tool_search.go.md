...
`tool_search.go` is Knowledge's first predefined tool binding. Its factory closes over the current Project identifier, validates a small public query shape, calls ordinary Knowledge retrieval, and returns citation-bearing regions without exposing local storage identities or a model-selectable Project field.

## Code breakdown

### Project-bound Knowledge search binding
...
The handler's closure is the Project boundary: an argument cannot name another Project, and unknown fields such as `projectId` are rejected before retrieval.
