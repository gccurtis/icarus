# src/lib/systems/documents/comments.ts — breakdown

Companion to [comments.ts](comments.ts). The anchored-comment client (Omega
`opts.Comments`) — threads pinned to a document anchor, with replies + a resolved
state. Replaces the mock comments (Goal B3).
...
`DocumentComment` / `CommentReply` mirror Omega's records. `CommentAnchor` is the inline
anchor a new comment carries (a block, optionally an atom range).
...
`GET /documents/:id/comments` (optional `?resolved=` filter).

## Create / reply / patch / delete
...
`createComment` posts a body + inline anchor; `replyToComment` appends a reply;
`patchComment` resolves/reopens or edits; `deleteComment` removes a thread. Each maps 1:1
to an Omega comment route.
