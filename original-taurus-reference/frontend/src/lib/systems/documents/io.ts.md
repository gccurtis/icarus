# src/lib/systems/documents/io.ts — breakdown

Companion to [io.ts](io.ts). Document import / export (Markdown) against Omega.
...
`exportDocumentMarkdown` uses a raw `fetch` (not the JSON `api` helper) because Omega's
...
Builds a Blob and clicks a transient anchor to download `<name>.md`.

## Import (upload → create)
...
`fileToBase64` encodes the file bytes for the `/files` upload; `importMarkdownFile`
runs the two-step flow (upload → `/documents/import` by file id) and returns the new
document's id + name for the caller to open as a tab.
