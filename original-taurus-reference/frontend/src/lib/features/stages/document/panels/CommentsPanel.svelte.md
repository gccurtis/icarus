# src/lib/features/stages/document/panels/CommentsPanel.svelte — breakdown

Companion to [CommentsPanel.svelte](CommentsPanel.svelte). The document's **real**
anchored-comment thread view (Goal B3): compose a comment on the selected block, list
threads, reply, and resolve/reopen — backed by Omega's comment routes.

## Script — imports, state, and the anchor target

### Imports, comment state, and the selection-derived anchor target
...
Derives the open document id + the **anchor target** (the first block of the current editor
selection) so a new comment pins to it. `canComment` gates the composer on having a block.

## Script — load + actions

### The load effect and the create/reply/resolve actions
...
An effect (re)loads comments when the document changes. `addComment`/`sendReply`/`toggleResolved`
call the real comment client and reload; each guards on `busy`.

## Markup — composer + open/all filter

### New-comment composer and the open/all filter
...
The composer (disabled until a block is selected) + the Open/All filter. The Mock badge is gone.

## Markup — thread list

### Loading/empty states and the expandable comment threads
...
Loading / error / empty / populated states. Each collapsed row shows the author, relative time,
reply count, and a preview; expanding reveals the body, an orphaned-anchor notice, the replies, a
reply box, and a Resolve/Reopen toggle.
