...
export const resolveSession = async (cookies: {
  ...
  return { userId: required(configuration, "development.userId") };
};
...
const projectForToken = async (
  ...
  const developmentUser = required(configuration, "development.userId");
  const developmentToken = required(configuration, "development.projectToken");

  return userId === developmentUser && projectToken === developmentToken
    ? required(configuration, "development.projectId")
    : undefined;
};
 ...
 */
const configurationOf = async (): Promise<Configuration> =>
  (await serverRuntime()).configuration;

/**
 * The pre-authentication identity, read from configuration rather than
 * hardcoded, so the one place naming it is a file someone can find.
 */
const required = (configuration: Configuration, key: string): string => {
  const value = configuration.get(key);
  if (typeof value !== "string" || value.length === 0) {
    throw new Error(
      `Configuration key '${key}' must be a non-empty string — see configuration/dev.yaml`
    );
  }
  return value;
};
