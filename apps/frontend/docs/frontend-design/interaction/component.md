# Interaction — component contract

> **Committed stance.** What every component must satisfy, whatever library
> ultimately provides it. The disclosure discipline it sits inside is
> [theory](theory.md).
...
- **Never icon-only for meaningful live state.** A state a user must act on gets
  words. This is the [color law](../color/roles.md#usage-laws) applied to glyphs:
...
Every token below names a [slot](../color/slots.md), never an intensity.

| State | Meaning | Required cues |
| --- | --- | --- |
| **Idle** | At rest, available | Neutral boundary, no fill |
| **Hover** | Pointer is over it | A border or background shift only — `--color-*-surface-hover` behind content, `--color-*-fill-hover` on a solid. Never a change in size or position |
| **Focus** | Keyboard focus | `focus-ring`: 2px `--color-interactive-border`, 2px offset. Always visible, never suppressed |
| **Selected** | This is the current object | `--color-active-text` on the label, `--color-active-border` on the control or marker; persistent; distinguishable from hover with color removed |
| **Active** | Being pressed right now | Momentary feedback at `--motion-micro` |
| **Disabled** | Unavailable | `--color-inactive-text` **plus a reason** on hover or focus. Never a silently dead control |
| **Pending** | Queued, not started | Copy naming the queue position or wait. No motion |
| **Resolving** | Working now | `--color-active-text` + progress + copy naming the current stage |
| **Applied** | Committed and in effect | `--color-success-text` + icon + copy |
| **Failed** | Did not complete | `--color-danger-text` + icon + copy + a recovery action |
| **Needs review** | Waiting on human judgment | `--color-attention-text` + icon + copy + an explicit review affordance |
| **Stale** | Its source moved underneath it | `--color-attention-text` + copy naming what changed + a refresh path |
...
- **Disabled explains itself.** A control that cannot be used says what would
  ...
  cheapest to fix.
- **Failure offers a way forward.** `--color-danger-text` without a recovery
