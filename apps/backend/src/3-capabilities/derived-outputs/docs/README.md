...
The capability owns saved output definitions, immutable answer revisions, refresh attempts, freshness, evidence validation, optional idempotency claims, and refresh settlement. Knowledge owns source ingestion and retrieval. Intelligence owns provider execution. Resource capabilities own source content and the places where an output reference is presented.
...
| [Flows](flows.md) | All six HTTP jobs, full refresh sequences, tools, invalidation, and status mapping |
...
- Platform [`Knowledge`](../../../0-platform/knowledge/knowledge.ts), including frozen scope manifests and source-mutation events.
- Platform [`Intelligence`](../../../0-platform/intelligence/intelligence.ts) for structured planning and tool-using synthesis.
- The composition-layer [`RuntimeResourceRegistry`](../../../1-init/create/resource-reader.ts), implementing both Knowledge resolution and scoped resource list/read.
...
| SQLite schema, claims, CAS settlement/invalidation/deletion | [`sqlite-store.ts`](../sqlite-store.ts) |
