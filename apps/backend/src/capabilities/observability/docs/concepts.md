...
Configuration loading occurs before logger creation, so configuration-read/parse failures cannot use this logger. Directory creation failure aborts logger construction/startup. Once constructed, [`create-runtime.ts`](../../../initialization/create-runtime.ts) wraps the remaining composition/listen path and logs `backend.start.failed` before rethrowing.
