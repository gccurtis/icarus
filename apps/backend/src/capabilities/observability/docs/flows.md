...
Every registered HTTP endpoint passes through [`registerHttpTransport.ts`](../../../api/registerHttpTransport.ts). Endpoint-specific logging may occur inside the Job/capability between scheduler events.
