...
- content, atom, mark, and list identifiers, all generated by the capability;
...
| `RichContentIdFactory` | internal | Allocates every content, atom, mark, and list ID. | [id-factory.md](runtime-objects/id-factory/id-factory.md) |

The ID factory is injected into the runtime and never leaves the capability, so
it has a `runtime-objects/` directory and no `runtime-api/` directories.
...
- Every content, atom, mark, and list ID is generated by Rich Content. A
  consumer never supplies one.
