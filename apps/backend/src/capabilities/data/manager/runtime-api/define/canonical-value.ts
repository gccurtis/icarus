...
import { canonicalDate } from "#data-manager/runtime-api/define/canonical-date.js";
import {
  invalidValue,
  isRecord
} from "#data-manager/runtime-api/define/value-guards.js";
import { canonicalName } from "#data-manager/runtime-api/shared/canonical-name.js";
import type { Field, ValueType } from "#data-manager/types/schema.js";
import type {
  DataRecord,
  DataValue
...
const withAncestor = (
  ...
  if (ancestors.has(input)) {
    throw new DataManagerError("invalid-value", `${path} contains a value cycle`);
  }
...
const canonicalRecord = (
  ...
  if (missing || extra) {
    ...
    throw new DataManagerError("invalid-schema", `${path} has ${detail}`);
  }
