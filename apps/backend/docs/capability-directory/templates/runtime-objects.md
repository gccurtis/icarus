...
An **exported** object is re-exported from `index.ts`; the composition root and
other capabilities hold it, and every method on its interface has a `runtime-api`
...
{{The order `build-runtime.ts` must construct these in, and what each
construction requires to already exist.}}
