...
[`createKnowledge`](../../../initialization/runtimes/knowledge.ts) is the production factory. It always supplies `./data/knowledge.db`. The adapter has a public `close()` method, but `KnowledgeStore` does not declare it and startup currently retains no handle for shutdown closure.
