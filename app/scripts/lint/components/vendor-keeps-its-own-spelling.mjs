...
export default check({
  ...
  subjects: {
    "import-spelling": "a vendored import is relative or $lib/components/vendor/…, which is what the CLI writes",
  ...
  run(tree) {
    ...
    const expected = "$lib/components/vendor";
    ...
    for (const path of tree.under(tree.path("components", "vendor"))) {
      ...
      for (const record of tree.imports(path)) {
        ...
        if (!resolved || !tree.within(tree.path("components", "vendor"), resolved)) continue;
