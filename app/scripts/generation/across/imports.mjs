 ...
 * file: `src/lib/views/panels/x.svelte` is `$panels/x.svelte`, not
 * `$views/panels/x.svelte`.
 */
...
const canonical = (file) => {
  for (const { alias, root } of targets) {
    ...
    return spelled.endsWith("/index") ? `${alias}/${spelled.slice(0, -"/index".length)}` : `${alias}/${spelled}`;
  }
  return null;
};
