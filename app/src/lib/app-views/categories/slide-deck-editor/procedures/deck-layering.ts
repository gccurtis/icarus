...
import {
  ...
  withoutElements
} from "$app-views/categories/slide-deck-editor/procedures/deck-elements";
import { boundsOf, relativeTo, within } from "$app-views/categories/slide-deck-editor/procedures/deck-geometry";
...
export const withRestacked = (body: SlideDeckBody, id: string, way: Restack): Edit => {
  ...
  if (after === wasAfter) return noDeckEdit(body);
...
export const withRestackedSet = (body: SlideDeckBody, ids: readonly string[], way: Restack): Edit => {
  const first = ids[0];
  if (first === undefined) return noDeckEdit(body);
...
export const withReorderedElement = (body: SlideDeckBody, id: string, after: string | null): Edit => {
  ...
  const siblings = siblingsOf(slide, placed.parents);
  const wasAfter = idBefore(siblings, id);
  if (after === id || after === wasAfter) return noDeckEdit(body);
  if (after !== null && !siblings.some((element) => element.id === after)) return noDeckEdit(body);
  return deckEdit(body, [
    { op: "move", target: "element", path: listPathFor(slide, placed.parents), id, after, wasAfter }
  ]);
};
