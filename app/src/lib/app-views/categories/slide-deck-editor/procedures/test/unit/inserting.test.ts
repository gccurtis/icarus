...
describe("what a fresh object is", () => {
  ...
  it("gives a text box the deck's default style and a height that follows its text", () => {
  ...
  it("mints an id nothing else on the slide holds", () => {
    const deck = body();
    const one = makeElement("rectangle", deck, frameFor("rectangle"));
    const two = makeElement("rectangle", deck, frameFor("rectangle"));
    expect(one.id).not.toBe(two.id);

    const edit = withInsertedElements(deck, "s1", [one, two]);
