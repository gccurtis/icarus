...
describe("deck named styles", () => {
  it("projects the list and a concise visual summary", () => {
    const deck = body();
    expect(styleOptions(deck)).toEqual([
      { value: "body", label: "Body" },
      { value: "caption", label: "Caption" }
    ]);
    expect(styleSummary(deck.styles.styles.caption)).toBe("IBM Plex Sans 14/1.3 · italic");
  });

  it("edits fields idempotently and can change the default", () => {
    const deck = body();
    expect(styleFieldEdit(deck, "caption", "fontSize", 14).ops).toEqual([]);

    const resized = styleFieldEdit(deck, "caption", "fontSize", 16);
    ...
    expect(defaultStyleEdit(deck, "missing").ops).toEqual([]);
  });

  it("creates and duplicates collision-free styles from represented values", () => {
    const deck = body();
    const made = newStyleEdit(deck);
