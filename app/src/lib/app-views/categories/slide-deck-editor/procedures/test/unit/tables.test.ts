...
const deck = (block: TableBlock): SlideDeckBody => ({
  aspectRatio: "16:9",
...
const tableIn = (body: SlideDeckBody): TableBlock => {
...
describe("merging and splitting", () => {
  it("merges a rectangle into its top-left cell and drops the rest", () => {
    const merged = withMergedCells(deck(table()), table(), ["a1", "b1", "a2", "b2"]);
  ...
  it("refuses to merge an L", () => {
    expect(withMergedCells(deck(table()), table(), ["a1", "b1", "a2"]).ops).toEqual([]);
  });

  it("keeps the first-picked cell and moves it to the top-left of the rectangle", () => {
    const merged = withMergedCells(deck(table()), table(), ["b2", "a1", "b1", "a2"]);
    const next = tableIn(merged.body);
