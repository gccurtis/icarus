<script lang="ts">
  ...
  const deckAnchorOf = (held: CommentThread): { slideId?: string; elementId?: string } => {
...
<Panel title="Comment">
  {#snippet crumbs()}<PanelCrumbs trail={[{ label: "Deck" }, { label: "Comment" }]} onnavigate={() => {}} />{/snippet}
  {#snippet actions()}
    {#if canLocate}<PanelButton label="Show in deck" icon={Locate} tone="ghost" onclick={locate} />{/if}
  {/snippet}
