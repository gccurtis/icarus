<script lang="ts">
  ...
  const restructure = (edit: { ops: readonly Parameters<SlideDeckRuntime["apply"]>[0][number][] }, keep: boolean) => {
    if (edit.ops.length === 0) return;
    runtime?.apply(edit.ops);
    if (!keep) backToTable();
  };
...
<Panel title={single ? "Cell" : `${chosen.length} cells`}>
  {#snippet crumbs()}
    <PanelCrumbs
      trail={[{ label: `Slide ${position}`, key: "slide-deck-editor.slide" }, { label: "Table", key: "slide-deck-editor.table" }, { label: single ? "Cell" : "Cells" }]}
      onnavigate={(key) => {
        if (key === "slide-deck-editor.slide" && slide) view.inspect("slide-deck-editor.slide", slideSignal(slide.id).selection);
        if (key === "slide-deck-editor.table") backToTable();
      }}
    />
  {/snippet}
  {#snippet actions()}
