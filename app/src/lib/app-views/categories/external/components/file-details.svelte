...
<dl>
  ...
  <dt>References</dt><dd>{file.usage.total}</dd>
</dl>

<style>
  dl { display: grid; grid-template-columns: minmax(0, 4.5rem) minmax(0, 1fr); gap: calc(var(--token-spacing-unit) * 1.5) calc(var(--token-spacing-unit) * 2); margin: 0; font-size: var(--token-text-caption); line-height: var(--token-text-caption-leading); }
