<script lang="ts">
  ...
  import {
    externalDirectoryIsSelected,
    externalFileIsSelected,
    KIND_LABEL
  } from "$app-views/categories/external/procedures/library-view";
  import { enterLibraryDirectory } from "$app-views/categories/external/procedures/enter-library-directory";

  const context = externalLibraryContext();
  const { state, view } = context;
  const directories = $derived(context.directDirectories());
