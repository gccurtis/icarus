<script lang="ts">
  ...
  const knownBytes = $derived(files.reduce((total, file) => total + (file.size ?? 0), 0));
  const unknownSizes = $derived(files.filter((file) => file.size === null).length);
    ...
    <div class="stack">
      ...
      <section>
        ...
        <dl>
          ...
          <dt>Legacy size unknown</dt><dd>{unknownSizes}</dd>
