...
The three state modules declare instance fields only. Components construct that
state, category-owned contexts make it available to cohesive children, named
procedures perform commands and transitions, and effect modules own reactive
lifecycle work. No state module performs a capability call.

`content/library.svelte` composes uploads, controls, filters, and file/directory
tables. `inspector/file.svelte` composes details, semantic status, and references;
the directory inspector follows the same state → procedure → effect boundary.
Small components receive identity or category context rather than mutable state
