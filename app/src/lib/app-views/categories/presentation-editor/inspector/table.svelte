<script lang="ts">
  ...
  import { columnsOf, gridOf, withColumnInserted, withColumnRemoved, withRowInserted, withRowRemoved } from "$app-views/categories/presentation-editor/procedures/tables";
  ...
  const change = (edit: { ops: readonly Parameters<PresentationRuntime["apply"]>[0][number][] }) => {
...
<Panel title="Table">
  {#snippet crumbs()}
    <PanelCrumbs
      trail={[{ label: "Presentation" }, { label: `Slide ${position}`, key: "presentation-editor.slide" }, { label: "Table" }]}
