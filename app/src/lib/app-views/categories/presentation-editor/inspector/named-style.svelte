<script lang="ts">
  ...
  import { swatchesFor, withNone } from "$app-views/categories/presentation-editor/procedures/palette";
  import {
    defaultStyleEdit,
    deleteStyleEdit,
    duplicateStyleEdit,
  ...
  const commit = (ops: Parameters<PresentationRuntime["apply"]>[0]) => {
  ...
  const duplicate = () => {
    ...
    if (made.key !== undefined) view.inspect("presentation-editor.named-style", { kind: "named-style", id: made.key });
  };

  const remove = () => {
    if (body === undefined) return;
    commit(deleteStyleEdit(body, key).ops);
    view.selectContext("presentation-editor.theme");
    view.clear();
  };
</script>
...
<Panel title={style?.name ?? "Style"}>
  ...
  {#snippet crumbs()}
    <PanelCrumbs trail={[{ label: "Presentation" }, { label: "Style" }]} onnavigate={() => {}} />
  {/snippet}
  ...
  {#if style === undefined}
    <div class="pt-2"><PanelNote tone="muted">The presentation has no style called {key}.</PanelNote></div>
  {:else}
