<script lang="ts">
  ...
  import { elementsSignal, slideSignal } from "$app-views/categories/presentation-editor/procedures/selecting";
  ...
  const threads = $derived(
    threadsIn(commentFeed)
      .filter((thread) => thread.target.kind === "presentation" && thread.target.id === presentationId)
  ...
  const post = () => {
    ...
    if (presentationId === undefined || anchor === undefined || text === "" || posting) return;
...
<Panel title="Comments on {subject}">
  {#snippet crumbs()}
    <PanelCrumbs
      trail={[{ label: `Slide ${position}`, key: "presentation-editor.slide" }, ...(element ? [{ label: labelOf(element), key: "presentation-editor.shape" }] : []), { label: "Comments" }]}
      onnavigate={(key) => {
        if (key === "presentation-editor.slide" && slide) view.inspect("presentation-editor.slide", slideSignal(slide.id).selection);
        if (key === "presentation-editor.shape" && element) { const signal = elementsSignal([element]); if (signal) view.inspect(signal.key, signal.selection); }
      }}
    />
  {/snippet}
  {#if workingCopy}
