<script lang="ts">
  ...
  import { notesBlock, slideIndexOf, withNotes } from "$app-views/categories/presentation-editor/procedures/presentation-slides";
  import { colorOf, textSceneOf } from "$app-views/categories/presentation-editor/procedures/scene";
...
<Panel title="Speaker notes">
  {#snippet crumbs()}
    <PanelCrumbs
      trail={[{ label: "Presentation" }, { label: `Slide ${position}`, key: "presentation-editor.slide" }, { label: "Notes" }]}
  ...
  {#if body && slide}
    ...
    {/if}
  {:else}
    <PanelEmpty title="Open a presentation to write speaker notes" />
  {/if}
</Panel>
