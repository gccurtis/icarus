...
const replacePresentation = (block: EditableTextBlock, from: number, to: number, insert: string): PresentationOp[] => {
  ...
  const textOp = (atomId: string, at: number, text: string, remove: string): PresentationOp => ({
    op: "text",
    target: "atom",
...
const promptEditLifecycleOps = (block: EditableTextBlock): PresentationOp[] => {
...
export const diffed = (block: EditableTextBlock, next: string): PresentationOp[] => {
...
export const toggledMark = (block: EditableTextBlock, from: number, to: number, style: MarkStyle): PresentationOp[] => {
  const start = Math.min(from, to);
  const end = Math.max(from, to);
  if (start === end) return [];
...
export const colouredMark = (block: EditableTextBlock, from: number, to: number, color: string | undefined): PresentationOp[] => {
  const start = Math.min(from, to);
  const end = Math.max(from, to);
  if (start === end) return [];
  const path = `${block.id}/marks`;
