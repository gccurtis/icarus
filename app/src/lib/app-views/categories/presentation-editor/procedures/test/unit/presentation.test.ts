...
import { withoutElements } from "$app-views/categories/presentation-editor/procedures/presentation-elements";
import { withGrouped, withRestacked, withRestackedSet, withUngrouped } from "$app-views/categories/presentation-editor/procedures/presentation-layering";
import { placedOn } from "$app-views/categories/presentation-editor/procedures/presentation-placement";
import { labelOf } from "$app-views/categories/presentation-editor/procedures/presentation-reading";
import { withNewSlide, withSavedLayout } from "$app-views/categories/presentation-editor/procedures/presentation-slide-layouts";
import { stepped, withDuplicatedSlide, withMovedSlide, withoutSlide } from "$app-views/categories/presentation-editor/procedures/presentation-slide-order";
import { valueAt, withElementFrame, withSet } from "$app-views/categories/presentation-editor/procedures/presentation-values";
...
const presentation = (over: Partial<PresentationBody> = {}): PresentationBody => ({
  aspectRatio: "16:9",
...
const ids = (body: PresentationBody) => body.slides.map((slide) => slide.id);

describe("presentation", () => {
  it("a new slide takes the layout of the one it follows", () => {
    const next = withNewSlide(presentation(), "s1");
  ...
  it("a new slide from a named layout gets its placeholders as text boxes", () => {
    const next = withNewSlide(presentation(), "s3", "title-only");
  ...
  it("a duplicate shares no id with its original", () => {
    const next = withDuplicatedSlide(presentation(), "s1");
  ...
  it("removing the first slide re-anchors its section", () => {
    const next = withoutSlide(presentation(), "s1");
    expect(ids(next.body)).toEqual(["s2", "s3"]);
    expect(next.body.sections[0].firstSlideId).toBe("s2");
  });

  it("a presentation keeps its last slide", () => {
    const one = presentation({ slides: [presentation().slides[0]], sections: [] });
    expect(withoutSlide(one, "s1").ops).toEqual([]);
  });

  it("moves and steps", () => {
    expect(ids(withMovedSlide(presentation(), "s3", null).body)).toEqual(["s3", "s1", "s2"]);
    expect(ids(stepped(presentation(), "s3", "up").body)).toEqual(["s1", "s3", "s2"]);
    expect(stepped(presentation(), "s1", "up").ops).toEqual([]);
  });

  it("saves a slide as a layout that locks a copy of everything on it", () => {
    const saved = withSavedLayout(presentation(), "s1", " Board opener ");
...
describe("elements", () => {
  it("sets a frame by the element's own id", () => {
    const next = withElementFrame(presentation(), "e2", frame(0.5));
  ...
  it("a set that changes nothing is no op", () => {
    expect(withSet(presentation(), "element", "e2/frame", frame(0.4)).ops).toEqual([]);
    expect(valueAt(presentation(), "theme/colors/text")).toBe("--token-ink-primary");
  });

  it("restacks within the slide", () => {
    const front = withRestacked(presentation(), "e1", "front");
  ...
  it("labels an element by what it is", () => {
    const [textBox, rect] = presentation().slides[0].elements;
