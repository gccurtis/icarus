...
import type { TextStyle } from "$representation/data/types/presentations/style-set";
...
import { FAMILIES } from "$app-views/categories/presentation-editor/procedures/palette";

export type { TextStyle } from "$representation/data/types/presentations/style-set";

export const styleOptions = (body: PresentationBody | undefined): readonly { value: string; label: string }[] =>
...
export const defaultStyleEdit = (body: PresentationBody, key: string): Edit =>
  body.styles.styles[key] === undefined || body.styles.defaultKey === key
    ? { body, ops: [] }
    : withSet(body, "presentation", "styles/defaultKey", key);
...
export const newStyleEdit = (
  ...
  return {
    key,
    edit: withSet(body, "presentation", `styles/styles/${key}`, { ...base, name: "New style" })
  };
};
...
export const duplicateStyleEdit = (
  ...
  return { key: made, edit: withSet(body, "presentation", `styles/styles/${made}`, { ...style, name }) };
};

const textBlocksIn = (body: PresentationBody): readonly (TextBlock | PromptBlock)[] => {
...
export const deleteStyleEdit = (body: PresentationBody, key: string): Edit => {
  ...
  return withSets(body, [
    ...references,
    { target: "presentation", path: `styles/styles/${key}`, value: null }
  ]);
};
