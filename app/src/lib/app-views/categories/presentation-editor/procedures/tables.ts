...
const none = (body: PresentationBody): Edit => ({ body, ops: [] });

const finished = (body: PresentationBody, ops: readonly PresentationOp[]): Edit =>
...
const removals = (table: TableBlock, ids: readonly string[]): PresentationOp[] => {
...
export const withMergedCells = (body: PresentationBody, table: TableBlock, ids: readonly string[]): Edit => {
...
export const withSplitCell = (body: PresentationBody, table: TableBlock, id: string): Edit => {
  const grid = gridOf(table);
...
export const withRowInserted = (body: PresentationBody, element: SlideElement, afterIndex: number): Edit => {
  ...
  const ops: PresentationOp[] = [{ op: "insert", target: "block", path: `${table.id}/rows`, ids: [row.id], after: anchor, values: [row] }];
...
export const withRowRemoved = (body: PresentationBody, element: SlideElement, index: number): Edit => {
  const held = tableOf(element);
  if (held === undefined) return none(body);
  const { table, rowHeights } = held;
...
export const withColumnInserted = (body: PresentationBody, element: SlideElement, afterColumn: number): Edit => {
  const held = tableOf(element);
  if (held === undefined) return none(body);
  const { table } = held;
  const grid = gridOf(table);
  const columns = columnsOf(grid);
  const ops: PresentationOp[] = table.rows.map((row) => {
    const made = blank();
...
export const withColumnRemoved = (body: PresentationBody, element: SlideElement, column: number): Edit => {
  const held = tableOf(element);
  if (held === undefined) return none(body);
  const { table } = held;
  const grid = gridOf(table);
