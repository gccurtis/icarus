<script lang="ts">
  ...
  import { withReorderedElement, withRestackedSet } from "$app-views/categories/presentation-editor/procedures/presentation-layering";
  ...
  import { elementsSignal, selectedIds } from "$app-views/categories/presentation-editor/procedures/selecting";
...
<Panel title="Layers">
  {#if body && slide}
    ...
    {/each}
  {:else}
    <PanelEmpty title="Open a presentation to see what is on the slide" />
  {/if}
</Panel>
