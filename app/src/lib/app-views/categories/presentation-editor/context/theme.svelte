<script lang="ts">
  ...
  import { withSavedLayout, withoutLayout } from "$app-views/categories/presentation-editor/procedures/presentation-slide-layouts";
  ...
  import { newStyleEdit, styleOptions, styleSummary } from "$app-views/categories/presentation-editor/procedures/styles";
  import { sceneOf } from "$app-views/categories/presentation-editor/procedures/scene";
  import { asAspectRatio, ratioOf, ratioParts, slideUnits } from "$app-views/categories/presentation-editor/procedures/stage";
  ...
  const inspectedStyle = $derived(view.inspected === "presentation-editor.named-style" ? view.selection?.id : undefined);

  const set = (path: string, value: unknown) => {
    if (body === undefined) return;
    runtime?.apply(withSet(body, "presentation", path, value).ops);
  };
  ...
  const newStyle = () => {
    ...
    view.inspect("presentation-editor.named-style", { kind: "named-style", id: made.key });
  };
</script>
    ...
    <PanelSection title="Named styles" count={styles.length} flush>
      ...
      {#each styles as option (option.value)}
        ...
        <PanelRow
          ...
          onselect={() => view.inspect("presentation-editor.named-style", { kind: "named-style", id: option.value })}
        />
      {/each}
    </PanelSection>
  {:else}
    <PanelEmpty title="Open a presentation to style it" />
  {/if}
</Panel>
