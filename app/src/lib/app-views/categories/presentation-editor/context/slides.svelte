<script lang="ts">
  ...
  import type { PresentationBody } from "$app-views/categories/presentation-editor/procedures/presentation-types";
  import type { Edit } from "$app-views/categories/presentation-editor/procedures/presentation-edit";
  import { withNewSlide } from "$app-views/categories/presentation-editor/procedures/presentation-slide-layouts";
  import {
    stepped,
    withDuplicatedSlide,
    withMovedSlide,
    withoutSlide
  } from "$app-views/categories/presentation-editor/procedures/presentation-slide-order";
  ...
  import { ratioOf, slideUnits } from "$app-views/categories/presentation-editor/procedures/stage";
  ...
  const show = (slideId: string) => {
    ...
    view.open({ category: "presentation-editor", resourceId: presentationId, focus: slideId });
    view.inspect("presentation-editor.slide", slideSignal(slideId).selection);
  };
  ...
  const minted = (before: PresentationBody, edit: Edit): string | undefined =>
...
<Panel title="Slides">
  {#snippet actions()}
    {#if body}
      <div class="verbs border-border-subtle flex w-full items-stretch gap-1 border-b pb-2">
        ...
        <Button variant="destructive" size="xs" class="flex-1" disabled={last} title={last ? "A presentation keeps at least one slide" : "Delete slide"} aria-label="Delete slide" onclick={remove}>
  ...
  {:else}
    <PanelEmpty title="Open a presentation to see its slides" />
  {/if}
</Panel>
