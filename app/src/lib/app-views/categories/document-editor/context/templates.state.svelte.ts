...
type Context = {
  ...
  readonly setNames: () => ScopeNames;
};

/** Component-instance state and asynchronous commands for the document Templates panel. */
export class TemplatesContextState {
  ...
  private async settled(): Promise<boolean> {
    ...
    if (runtime.pending > 0 || runtime.failure !== undefined) {
      this.actionError = "The document has changes that are not saved yet. Save them first.";
      return false;
    }
    return true;
  }
  ...
  save(): void {
    void this.run("save", async () => {
      const documentId = this.context.documentId();
  ...
  saveAs(): void {
    void this.run("save-as", async () => {
      ...
      if (!(await this.settled())) return;
      const made = await saveAsTemplate(this.context.view, documentId, name);
      if (!this.live) return;
      if (!made.accepted) {
  ...
  private async place(
    ...
    const insertion = insertionOf(
      body,
      detail,
      currentRowId(body, this.context.view.selection),
      stage === undefined ? "resolve" : "keep",
      answers,
      words
    );
    if (insertion.ops.length === 0) {
      this.notice = ["That template has no content to insert."];
      return;
    }
    ...
    if (stage !== undefined && template !== undefined) {
      ...
      if (merged.length !== template.holes.length) {
        const result = await updateHoles(this.context.view, template, merged, documentId);
