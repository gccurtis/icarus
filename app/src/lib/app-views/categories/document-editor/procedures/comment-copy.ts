...
import type { Person, Remark } from "$app-views/categories/document-editor/procedures/comments";

export const nameOf = (users: readonly Person[], actor: CommentActor | undefined): string => {
  if (actor === undefined) return "Someone";
  if (actor.kind !== "user") return actor.kind === "connector" ? "A connector" : "An agent";
