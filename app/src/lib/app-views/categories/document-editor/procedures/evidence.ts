...
/** Evidence links use the compact clock precision used elsewhere in the editor. */
export const compactEvidenceSourceTitle = (value: string): string =>
  value.replace(/\b(\d{1,2}:\d{2}):\d{2}(\s+[AP]M)\b/i, "$1$2");

/**
 * Resource names used to be prepended to the exact-text projection. Keep old
 * citations readable without presenting that navigation metadata as evidence.
 */
