 ...
 */
export const recalculatesOnVariables = (register: VariableRegister, open: () => OpenSheet): void => {
  $effect(() => {
    void register.revision;

    untrack(() => {
      const { resourceId, sheet, runtime } = open();
      if (sheet === undefined || resourceId === undefined) return;

      const ops = recalculated(sourceFor(resourceId, sheet), aroundOf(register));
