...
Lives at `src/lib/development-views/thread-demo/thread-demo.md`. Trees live in the concern
...
- `$components/authored/panel` — the message parts and the supporting panel
- `$components/authored/screen` — the plane the thread is set on
- `$lib/components/vendor/textarea` — the composer's field
