...
sheet view → renders constants, not SpreadsheetRuntime.body`,
  ...
  checkers: [
    ...
    },
    {
      ...
      status: "Enforced",
      wave: 1,
      mechanism: "Production AST + source-path vocabulary + exact admission gates",
      guarantee: "Production executable and type syntax contains no explicit legacy, compatibility, deprecated, or migration path; registered retired fields cannot return under neutral names; registered required current fields cannot become optional or be repaired when absent; shared represented-value gates reject custom prototypes, accessors, explicit undefined, and every hidden or symbolic own field; represented resource kinds and references stay closed and nominal; Document and Presentation read/submit plus Variable read/save/remove commands require exact plain inputs, nominal ids, exact operation/value arms, and coherent path claims; stored messages are complete, authored, and error-free; semantic materials are ready-only while retaining only their optional current degradation error; formula atoms and blocks are fresh resolved snapshots with no retired work/error fields and no FormulaBlock.resolvedAt; semantic text citations name their exact arm and generated material descriptors alone require model provenance; Derived Outputs explicitly distinguish none, authored, and generated values and constrain each state plus refresh/semantic job to one complete arm; a prompt block has exactly one definition owner and one complete unlinked-idle or linked idle/stale/fresh/error lifecycle arm; Research commands admit exact own keys and nominal ids; Research turns and Agent tasks use complete discriminated lifecycle arms; running Agent tasks name an explicit executor; Agent commands require nominal ids, exact own keys, canonical unique values, and exact schedule/reference variants; Agent name projections are project-bounded; and native External storage references and claims reject extra own fields.",
      detects: "Legacy-prefixed types and constants, compatibility or migration source paths, executable fallback discriminator strings, retired members on current schema types, optional or missing registered required fields, defaults applied to path-scoped current reads, projections that ignore custom prototypes, accessors, explicit undefined, non-enumerable fields, or symbols, Store admission bypasses, open Document/Presentation/Variable command envelopes, cross-kind resource ids, mixed operation/value arms, incoherent touched paths, an authorless, partial, or error-decorated thread message, a non-ready semantic material or a material carrying an old work marker, stale/computing/error formula snapshots or FormulaBlock.resolvedAt, an untagged or widened semantic text citation, missing generated material provenance or provenance on a non-generated facet, implicit or mixed Derived Output value ownership, an empty output claiming freshness, partial refresh/semantic jobs, a linked prompt retaining inline prompt/scope fields, an unlinked prompt claiming a linked state, a partial or mixed linked prompt lifecycle, an explicit-undefined prompt link, project-bound prompt links in templates, unknown or hidden Research command keys, non-nominal Research ids, mixed or partial Research-turn and Agent-task states, queued Research turns, plan-text executor inference, partial Agent startup, cross-project Agent labels, duplicate or reordered Agent tools/selectors, ambiguous schedule arms, explicit-undefined Agent refs, an optional or omitted slide set target, extra fields on native External storage references or claims, an open ResourceKind, a bare external-file reference alias, and ResourceRef kind/id namespace drift.",
      implementation:
        "Parse identifiers, string literals, and registered current reads from production TypeScript, Svelte scripts, and top-level operational scripts; inspect source-path components; compare current schema types with explicit retired-member, required-member, required-literal, exact-arm, exact-union, and path-plus-read registries; and require shared represented values, the Store, plus registered nested-value, lifecycle, executor, prompt-owner, editor/variable-command, Research-command, Agent-command, semantic-facet, and native-storage admissions to retain their exact gates. Executable adversarial contracts and mutations prove each high-risk discriminator; comments and documentation do not count.",
      current: "Enforced; shared stored/content field admission, exact Document/Presentation/Variable commands, document leaders, template scopes and prompts, exact prompt owner/lifecycle arms, Research command inputs and turn lifecycle, Agent command inputs, task/question/executor lifecycle, two-phase startup and project-scoped names, slide operation targets, Derived Output value-source and job lifecycles, durable semantic jobs, complete authored messages, ready-only semantic materials, fresh-only formula snapshots, explicitly discriminated semantic citations with facet-exact provenance, spreadsheet snapshots and formulas, client storage, native External storage references and claims, every Store publication/recovery path, and the closed ResourceKind/ResourceRef contract reject hidden, incomplete, widened, lossy, or mismatched current shapes instead of repairing them.",
