<script lang="ts">
  import { noteLog } from "$development-views/agents-reference/shared/note-log.svelte";
  ...
  const send = async () => {
    ...
    if (!text || sending || log === undefined) return;
    sending = true;
...
{#if log}
  <div
    ...
    class:opacity-60={thread.length === 0 && !focused}
  >
    {#each thread as note (note.at)}
      <div class="border-l-intelligence-border bg-surface-panel rounded-control border-l-2 px-2.5 py-1.5">
        <p class="text-body-sm text-ink-primary m-0 break-words whitespace-pre-wrap">{note.text}</p>
        <span class="text-caption text-ink-muted tabular-nums">{stamp(note.at)}</span>
      </div>
    {/each}
