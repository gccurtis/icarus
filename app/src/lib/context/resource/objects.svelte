<script lang="ts">
  ...
  /**
   * Analytic components floating over the grid — the grid's equivalent of Layers.
   *
...
<Panel title="Objects">
  <PanelSection title="Analytic components" count={objects.length} flush>
    {#each objects as object (object.id)}
      <PanelRow
        title="{object.kind} analytic"
        sub={where(object)}
        meta={sizeOf(object)}
        icon={ICON[object.kind]}
        tone={object.overlapped ? "attention" : "default"}
        onselect={() =>
          view.inspect("resource.chart", { kind: "chart", id: object.id })}
      />
    {/each}
  </PanelSection>

  <PanelNote tone="gap">
    Each analytic, chart, datum, axis, and added element has a stable id. The list
    can be reordered without changing what an inspector, comment, or revision addresses.
  </PanelNote>
</Panel>
