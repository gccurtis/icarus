import { createStore, pathFor } from "$json-store/store.server";
import type { AgentTaskStatus, TaskPrompt } from "$json-store/types/agents/agent-task";
import type { Message } from "$json-store/types/agents/message";
import type { Cast, PersonaAvatar, PersonaDefinition } from "$json-store/types/agents/persona";
import type { BranchPoint, ThreadKind } from "$json-store/types/agents/thread";
import type { ActivityTarget } from "$json-store/types/collaboration/activity";
import type { AnchorWithin, Resolution } from "$json-store/types/collaboration/anchor";
import type { BlockFormat } from "$json-store/types/content/block-format";
import type { ContentBlock, Mark, MarkLink } from "$json-store/types/content/content-block";
import type { CellRef } from "$json-store/types/content/formula-value";
import type { VariableValue } from "$json-store/types/content/variable-value";
import type { MembershipRole } from "$json-store/types/core/access";
import type { Actor } from "$json-store/types/core/actor";
import type { Id, Row } from "$json-store/types/core/id";
import type { ResourceRef } from "$json-store/types/core/resource";
import type { ResourceSet } from "$json-store/types/core/resource-set";
import type { BackReferenceTargetKind } from "$json-store/types/data/back-reference";
import type { FormulaUse } from "$json-store/types/data/formula-use";
import type {
  ConnectionStatus,
  ConnectorCredential,
  ConnectorProvider,
  ConnectorStatus
} from "$json-store/types/external/connector";
import type {
  Dimensions,
  FileOrigin,
  FileSubkind,
  Readability
} from "$json-store/types/external/file";
import type { FindingSource } from "$json-store/types/investigation/finding";
import type {
  HypothesisAssessment,
  HypothesisEvidence
} from "$json-store/types/investigation/hypothesis";
import type { QuestionStatus, RelatedItem } from "$json-store/types/investigation/question";
import type { ResearchMode } from "$json-store/types/investigation/research-thread";
import type { DerivedEvidence, DerivedState } from "$json-store/types/knowledge/derived-output";
import type {
  LatticeBinding,
  LatticeCause,
  LatticeCluster,
  ...
  LatticeRemoval,
  LatticeWindow
} from "$json-store/types/knowledge/lattice";
import type { ResourceBody } from "$json-store/types/resources/resource-body";
import type { AspectRatio } from "$json-store/types/resources/slide-deck-body";
import type { GeneralResourceType, Op } from "$json-store/types/revisions/change";
import type { ChangeTier, SnapshotRole } from "$json-store/types/revisions/snapshot";
import type {
  TemplateBody,
  TemplateKind,
  TemplateVariable
} from "$json-store/types/templates/template";
...
export type JsonStoreInput = {
  /**
   * Where the tables live, one `<table>.json` per table. Absent keeps every
   * store in memory.
   */
  ...
  readonly dataBackReferences?: readonly DataBackReference[];
  readonly derivedOutputs?: readonly DerivedOutput[];
  readonly documents?: readonly Document[];
  readonly externalFiles?: readonly ExternalFile[];
  readonly findings?: readonly Finding[];
  readonly formulas?: readonly Formula[];
  readonly hypotheses?: readonly Hypothesis[];
  readonly latticeChanges?: readonly LatticeChange[];
  readonly latticeEdges?: readonly LatticeEdge[];
  readonly latticeNodes?: readonly LatticeNode[];
  readonly latticeSources?: readonly LatticeSource[];
  readonly memberships?: readonly Membership[];
  readonly personas?: readonly Persona[];
  readonly personaThreads?: readonly PersonaThread[];
  readonly projects?: readonly Project[];
  readonly questions?: readonly Question[];
  readonly researchThreads?: readonly ResearchThread[];
  readonly resourceSets?: readonly NamedResourceSet[];
  readonly resourceSnapshots?: readonly ResourceSnapshot[];
  readonly sheetCells?: readonly SheetCell[];
  readonly slideDecks?: readonly SlideDeck[];
  readonly spreadsheets?: readonly Spreadsheet[];
  readonly templates?: readonly Template[];
  readonly templateVersions?: readonly TemplateVersion[];
  readonly threadParts?: readonly ThreadPart[];
  readonly threads?: readonly Thread[];
  readonly users?: readonly User[];
  readonly variables?: readonly Variable[];
  /** Injected so a test can pin `_creationTime`. */
  readonly now?: () => number;
};

/**
 * Every table as one object, each store owning its own file. Returns a fresh set
 * each call and holds nothing.
 *
 * There are no indexes — `where` from `store.server.ts` is the scan that
 * replaces one.
 */
export const createJsonStore = ({ directory, now, ...rows }: JsonStoreInput = {}) => {
  const at = <Table extends string>(table: Table) => ({
    table,
    now,
    path: directory === undefined ? undefined : pathFor(directory, table)
  });

  return {
    activity: createStore<"activity", ActivityFields>({ ...at("activity"), initial: rows.activity }),
    agentTasks: createStore<"agentTasks", AgentTaskFields>({
      ...at("agentTasks"),
      initial: rows.agentTasks
    }),
    changeSets: createStore<"changeSets", ChangeSetFields>({
      ...at("changeSets"),
      initial: rows.changeSets
    }),
    comments: createStore<"comments", CommentFields>({ ...at("comments"), initial: rows.comments }),
    commentThreads: createStore<"commentThreads", CommentThreadFields>({
      ...at("commentThreads"),
      initial: rows.commentThreads
    }),
    connections: createStore<"connections", ConnectionFields>({
      ...at("connections"),
      initial: rows.connections
    }),
    connectors: createStore<"connectors", ConnectorFields>({
      ...at("connectors"),
      initial: rows.connectors
    }),
    dataBackReferences: createStore<"dataBackReferences", DataBackReferenceFields>({
      ...at("dataBackReferences"),
      initial: rows.dataBackReferences
    }),
    derivedOutputs: createStore<"derivedOutputs", DerivedOutputFields>({
      ...at("derivedOutputs"),
      initial: rows.derivedOutputs
    }),
    documents: createStore<"documents", DocumentFields>({
      ...at("documents"),
      initial: rows.documents
    }),
    externalFiles: createStore<"externalFiles", ExternalFileFields>({
      ...at("externalFiles"),
      initial: rows.externalFiles
    }),
    findings: createStore<"findings", FindingFields>({ ...at("findings"), initial: rows.findings }),
    formulas: createStore<"formulas", FormulaFields>({ ...at("formulas"), initial: rows.formulas }),
    hypotheses: createStore<"hypotheses", HypothesisFields>({
      ...at("hypotheses"),
      initial: rows.hypotheses
    }),
    latticeChanges: createStore<"latticeChanges", LatticeChangeFields>({
      ...at("latticeChanges"),
      initial: rows.latticeChanges
    }),
    latticeEdges: createStore<"latticeEdges", LatticeEdgeFields>({
      ...at("latticeEdges"),
      initial: rows.latticeEdges
    }),
    latticeNodes: createStore<"latticeNodes", LatticeNodeFields>({
      ...at("latticeNodes"),
      initial: rows.latticeNodes
    }),
    latticeSources: createStore<"latticeSources", LatticeSourceFields>({
      ...at("latticeSources"),
      initial: rows.latticeSources
    }),
    memberships: createStore<"memberships", MembershipFields>({
      ...at("memberships"),
      initial: rows.memberships
    }),
    personas: createStore<"personas", PersonaFields>({ ...at("personas"), initial: rows.personas }),
    personaThreads: createStore<"personaThreads", PersonaThreadFields>({
      ...at("personaThreads"),
      initial: rows.personaThreads
    }),
    projects: createStore<"projects", ProjectFields>({ ...at("projects"), initial: rows.projects }),
    questions: createStore<"questions", QuestionFields>({
      ...at("questions"),
      initial: rows.questions
    }),
    researchThreads: createStore<"researchThreads", ResearchThreadFields>({
      ...at("researchThreads"),
      initial: rows.researchThreads
    }),
    resourceSets: createStore<"resourceSets", NamedResourceSetFields>({
      ...at("resourceSets"),
      initial: rows.resourceSets
    }),
    resourceSnapshots: createStore<"resourceSnapshots", ResourceSnapshotFields>({
      ...at("resourceSnapshots"),
      initial: rows.resourceSnapshots
    }),
    sheetCells: createStore<"sheetCells", SheetCellFields>({
      ...at("sheetCells"),
      initial: rows.sheetCells
    }),
    slideDecks: createStore<"slideDecks", SlideDeckFields>({
      ...at("slideDecks"),
      initial: rows.slideDecks
    }),
    spreadsheets: createStore<"spreadsheets", SpreadsheetFields>({
      ...at("spreadsheets"),
      initial: rows.spreadsheets
    }),
    templates: createStore<"templates", TemplateFields>({
      ...at("templates"),
      initial: rows.templates
    }),
    templateVersions: createStore<"templateVersions", TemplateVersionFields>({
      ...at("templateVersions"),
      initial: rows.templateVersions
    }),
    threadParts: createStore<"threadParts", ThreadPartFields>({
      ...at("threadParts"),
      initial: rows.threadParts
    }),
    threads: createStore<"threads", ThreadFields>({ ...at("threads"), initial: rows.threads }),
    users: createStore<"users", UserFields>({ ...at("users"), initial: rows.users }),
    variables: createStore<"variables", VariableFields>({
      ...at("variables"),
      initial: rows.variables
    })
  };
};
