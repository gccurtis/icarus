...
import Person from "$inspector/collaboration/person.svelte";
import { createTrace, type TraceNode } from "$lib/trace/trace.svelte";
 ...
 */
const traced = (component: typeof Person) => {
...
describe("a run records what a panel is made of", () => {
  it("registers the primitives a lens rendered", () => {
    const { run } = traced(Person);
