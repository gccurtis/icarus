<script lang="ts">
  ...
  const lensFor = (cell: Cell | undefined) => {
    if (cell === undefined) return "resource.cell";
    if (cell.error !== undefined) return "resource.error-cell";
    if (cell.spillOrigin !== undefined && cell.spillOrigin !== cell.address) return "resource.spill";
    if (cell.formula !== undefined) return "resource.cell-with-formula";
          ...
          <div class="sheet bg-surface-panel">
            ...
            {#each charts as chart (chart.id)}
              <ChartElement
                ...
                selection={selectionFor(chart.id)}
                ...
                onselect={() => {
                  ...
                  mockWorkbench.inspect("resource.chart", { kind: "chart", id: chart.id });
                }}
              />
            {/each}
          </div>
