...
Nothing writes `documentChangeSets`. The call is written out in `flush.ts` as a
...
proved by its own tests instead. That is deliberate rather than an oversight:
the alternative was leaving the refusal path unwritten until the table lands, and
writing it against the vocabulary it will actually receive is what makes the
