 ...
 * conflict it created itself. Joining also means an awaiting caller learns when
 * the write actually settled, rather than returning immediately because someone
...
const submit = async (runtime: Runtime): Promise<void> => {
  ...
  try {
    // ── FORWARD DECLARATION ────────────────────────────────────────────────
    // `revisions.submit` does not exist: the capability ships its vocabulary
    // today and its tables later. This is the call we expect to run.
    //
    // ```ts
    // const accepted = await submitChangeSet({
    //   resourceType: runtime.type,
    ...
    // ```
    //
    // A refusal arrives as `RevisionsError` carrying the server's revision and
    // whether it is retryable, and is handed to `rebase` — which is written and
    // tested directly, because nothing here can produce one yet.
    //
    // Until the capability lands the accepted branch is taken locally. Every
    // state transition below this line is the real one: the buffer is consumed,
    // the revision advances, and the strip reports what it will report then.
