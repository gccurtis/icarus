 ...
 */
export const coalesce = (ops: readonly Op[]): Op[] => {
  const folded: Op[] = [];
  ...
  for (const op of ops) {
    ...
    const earlier = folded[previous] as Extract<Op, { op: "set" }>;
...
const lastSetOn = (ops: readonly Op[], path: string): number => {
...
const crossesRelatedGround = (ops: readonly Op[], from: number, path: string): boolean =>
