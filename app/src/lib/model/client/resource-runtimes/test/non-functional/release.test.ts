...
import { rebase } from "$model/client/resource-runtimes/methods/flush/rebase";
import { Runtime } from "$model/client/resource-runtimes/definition.svelte";
 ...
 *
 * `rebase` is exercised directly. Nothing can produce a refusal until
 * `revisions.submit` exists, and leaving the path unwritten until then would
...
const set = (path: string, value: number): Op => ({
  op: "set",
  target: "field",
  path,
  value,
...
test("releaseAll submits every open buffer", async () => {
  const runtimes = register();
  const document = runtimes.attach("document", "k57");
  const deck = runtimes.attach("slides", "s1");
  document.apply([set("a", 1)]);
  deck.apply([set("b", 2)]);

  runtimes.releaseAll();
  await vi.runAllTimersAsync();

  assert.equal(document.pending, 0);
  assert.equal(deck.pending, 0);
...
test("a refused change set goes back to the front of the buffer", () => {
  ...
  const runtime = new Runtime("document", "k57", { afterOps: 50, afterMs: 2000 });
  ...
  assert.equal(runtime.buffer[0].path, "refused");
  assert.equal(runtime.buffer[1].path, "typed-during-flight");
});
...
test("a runtime that cannot submit keeps reporting rather than disappearing", async () => {
  // Work disappearing is the one outcome with no recovery, so a failed submit
  // holds its buffer and its key stays visible.
  const runtimes = register();
  const runtime = runtimes.attach("document", "k57") as unknown as Runtime;
  runtime.apply([set("a", 1)]);
  runtime.sync = "error";
