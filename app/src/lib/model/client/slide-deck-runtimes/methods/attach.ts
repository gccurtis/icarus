...
/**
 * Open a deck, or hand back the one already open.
 *
 * **Idempotent, and that is the whole point.** The caller never has to know
 * whether it is the first viewer, which is what makes a second tab on one deck
 ...
 * deck whose release has not finished submitting is ordinary — close a tab and
 * reopen it inside two seconds — and minting a second runtime there would put
 * two buffers on one deck at exactly the moment one of them is mid-flush.
 *
 ...
 *
 * ── FORWARD DECLARATION ──────────────────────────────────────────────────────
 * Nothing serves a materialized deck body yet: `slideDecks` holds the body and
 * `slideDeckSnapshots` anchors the replay that produces it, and neither is
 ...
 *
 * ```ts
 * const subscription = readSlideDeck({ slideDeckId: runtime.id });
