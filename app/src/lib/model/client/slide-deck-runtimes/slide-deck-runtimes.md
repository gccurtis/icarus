...
Lives at the object root as `slide-deck-runtimes.md`. It is the entry point: a
...
```ts
runtime.body            // the whole current deck. Reactive. No call, no await.
...
into `SlideDeckOp[]` and hands them over. Buffering, coalescing, submitting and
...
- One runtime per open deck, keyed by **deck** and never by tab
...
- **Lifetime.** The workbench decides when a deck opens and closes
...
An *entry* lives from the first tab that opens a deck to the last one that closes
it.
...
| ------ | ----- | ------ | ----------- | -------- |
| `attach` | file | mutator | Open a deck, or hand back the one already open. Idempotent, never throws | [`methods/methods.md`](methods/methods.md) |
...
| `releaseAll` | file | mutator | The same, for every open deck | [`methods/methods.md`](methods/methods.md) |

A `SlideDeckRuntime` — what `attach` returns — offers `apply`, `flush`, `undo`
and `redo` over one deck. See [`methods/methods.md`](methods/methods.md).
...
| Field | Type | Meaning |
| ----- | ---- | ------- |
| `open` | `readonly string[]` | Every deck with a live runtime |
| `flushing` | `readonly string[]` | Those whose last submit has not settled |

Both are projections of deck ids. The maps behind them are private, because a
caller that could reach into one could hold a runtime past its release.

## Construction

```ts
export const createSlideDeckRuntimes = (configuration: ConfigurationModel): SlideDeckRuntimesModel => ...;
```
...
- **After release:** `attach` on the same deck opens a fresh runtime, unless the
...
- **One runtime per deck, never per tab.**
...
surface over it; `SlideDeckRuntimesState` holds the two maps; `SlideDeckRuntimes`
