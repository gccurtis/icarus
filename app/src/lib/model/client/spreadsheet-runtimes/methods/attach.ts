...
/**
 * Open a sheet, or hand back the one already open.
 *
 * **Idempotent, and that is the whole point.** The caller never has to know
 * whether it is the first viewer, which is what makes a second tab on one sheet
 * free — and what makes two tabs share one buffer rather than each holding half
 * the edits.
 *
 * A runtime found in `settling` is revived rather than duplicated. Reopening a
 * sheet whose release has not finished submitting is ordinary — close a tab and
 * reopen it inside two seconds — and minting a second runtime there would put
 * two buffers on one sheet at exactly the moment one of them is mid-flush.
 *
 ...
 *
 * ── FORWARD DECLARATION ──────────────────────────────────────────────────────
 * Nothing serves a materialized sheet body yet: `spreadsheets` holds the body,
 * `sheetCells` holds the cells and `spreadsheetSnapshots` anchors the replay
 * that produces both, and none is readable from here. The call below is the code
 * we expect to run, and everything around it is live — the runtime is genuinely
 * subscribed-shaped, holds the handle it will need to drop, and reports
 * `loading` until a body lands.
 *
 * ```ts
 * const subscription = readSpreadsheet({ spreadsheetId: runtime.id });
