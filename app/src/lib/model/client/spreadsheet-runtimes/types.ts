...
/**
 * One open sheet: what to render, and how to change it.
 *
 ...
 *
 * **`apply` is the only writer.** The editor translates a cell edit into
 ...
 * kept separate from `body` so a save completing does not re-render a sheet.
 */
...
/**
 * The register: one runtime per open sheet, keyed by **sheet** and never by tab.
 * Two tabs split across one sheet must write into one buffer, or each holds half
 ...
 * release.
 *
 * **The workbench owns lifetime.** It calls `attach` when a sheet tab opens and
 * `release` when the last tab on that sheet closes.
 */
export interface SpreadsheetRuntimesModel {
  /** Every sheet with a live runtime. */
