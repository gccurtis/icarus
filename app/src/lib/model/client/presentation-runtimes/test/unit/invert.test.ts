...
import { invert, invertAll } from "$model/client/presentation-runtimes/methods/history/invert";

test("a set exchanges value and was", () => {
  const op: PresentationOp = { op: "set", target: "slide", path: "slides/#s1", value: 3, was: 1 };
...
test("inverting never reads a value or resolves a path", () => {
  ...
  const inverted = invert(op) as Extract<PresentationOp, { op: "set" }>;
