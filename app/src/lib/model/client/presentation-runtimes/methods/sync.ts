...
import { emptyBody } from "$model/client/presentation-runtimes/methods/shared/empty-body";

/**
 * A runtime with nothing of its own outstanding is a reader, and only a reader
 * may take the leader's word for what the presentation says. Checked again after the
 ...
 * cache otherwise, and a presentation would never advance past the answer the page was
 * loaded with.
 */
export const sync = async (runtime: Runtime): Promise<void> => {
  ...
  try {
    const question = readPresentationBody({ resourceId: runtime.id });
