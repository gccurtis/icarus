# Shared Presentation Runtime Methods
...
| [`empty-body.ts`](empty-body.ts) | `attach` | A presentation the store has never held still opens on something |

## `emptyBody`

One blank slide, not zero. A presentation with no slides has nothing to draw and nothing
to select, so it is indistinguishable on screen from a presentation that failed to load —
