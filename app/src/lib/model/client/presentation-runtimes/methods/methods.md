...
| ------ | ----- | -------- | ------ | ----------- |
| `attach` | file | [`attach.ts`](attach.ts) | mutator | Open a presentation, or hand back the one already open |
| `release` | file | [`release.ts`](release.ts) | mutator | Detach one presentation and return it to be submitted |
| `releaseAll` | file | [`release-all.ts`](release-all.ts) | mutator | Detach every open presentation |
...
| Takes | Methods | Holds |
| --- | --- | --- |
| `PresentationRuntimesState` | `attach`, `release`, `releaseAll` | The two maps, and the thresholds |
| `Runtime` | `apply`, `flush`, `history` | One presentation's body, buffer, revision and stacks |
...
`PresentationRuntimesState.createRuntime` is the factory.

## The register is keyed by presentation id

There is nothing to build a key out of. One object holds presentations only, so the id
*is* the key.
...
schedules; `Runtime.undo` calls `history` and then buffers; `PresentationRuntimes.release`
