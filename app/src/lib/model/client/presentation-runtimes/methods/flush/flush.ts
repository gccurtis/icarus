import { submitPresentationChanges } from "$capabilities/presentation/index.remote";
...
import { rebase } from "$model/client/presentation-runtimes/methods/flush/rebase";
...
const changeSet = (runtime: Runtime, ops: readonly PresentationOp[]) => ({
...
const submit = async (runtime: Runtime): Promise<void> => {
  ...
  try {
    const answer = await submitPresentationChanges({ changeSet: changeSet(runtime, ops) });
    ...
    runtime.buffer = [];
    const retried = await submitPresentationChanges({ changeSet: changeSet(runtime, restated) });
