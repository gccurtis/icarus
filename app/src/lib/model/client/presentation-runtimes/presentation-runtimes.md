...
Lives at the object root as `presentation-runtimes.md`. It is the entry point: a
reviewer reads this, then follows the file tree into the document that answers
their question.

## Description

Presentation runtimes keep presentations in sync while somebody edits them. Companion to
...
```ts
runtime.body            // the whole current presentation. Reactive. No call, no await.
...
into `PresentationOp[]` and hands them over. Buffering, coalescing, submitting and
...
separate from `body` so a save completing does not re-render a presentation.

## Ownership Boundary

Presentation runtimes own:

- One runtime per open presentation, keyed by **presentation** and never by tab
...
- **Lifetime.** The workbench decides when a presentation opens and closes
...
An *entry* lives from the first tab that opens a presentation to the last one that closes
it.
...
| ------ | ----- | ------ | ----------- | -------- |
| `attach` | file | mutator | Open a presentation, or hand back the one already open. Idempotent, never throws | [`methods/methods.md`](methods/methods.md) |
...
| `releaseAll` | file | mutator | The same, for every open presentation | [`methods/methods.md`](methods/methods.md) |

A `PresentationRuntime` — what `attach` returns — offers `apply`, `flush`, `undo`
and `redo` over one presentation. See [`methods/methods.md`](methods/methods.md).
...
| Field | Type | Meaning |
| ----- | ---- | ------- |
| `open` | `readonly string[]` | Every presentation with a live runtime |
| `flushing` | `readonly string[]` | Those whose last submit has not settled |

Both are projections of presentation ids. The maps behind them are private, because a
caller that could reach into one could hold a runtime past its release.

## Construction

```ts
export const createPresentationRuntimes = (configuration: ConfigurationModel): PresentationRuntimesModel => ...;
```
...
- **After release:** `attach` on the same presentation opens a fresh runtime, unless the
...
- **One runtime per presentation, never per tab.**
...
Reading is built. `attach` loads one presentation's body through
`$capabilities/presentation`, and a presentation the store has never held opens on
`methods/shared/empty-body.ts` rather than on nothing — a presentation that draws nothing
cannot be told apart from one that is broken.

Writing is not. Nothing writes `presentationChangeSets`, so one call is still a
comment with the code we expect to run:

| Where | Call | What happens meanwhile |
| --- | --- | --- |
| `methods/flush/flush.ts` | append to `presentationChangeSets` | The accepted branch is taken locally |
...
`presentationSnapshots` is the server's replay anchor and appears nowhere here.

## File Tree

```text
presentation-runtimes/
├── presentation-runtimes.md
├── index.ts
├── types.ts
...
surface over it; `PresentationRuntimesState` holds the two maps; `PresentationRuntimes`
