...
timers for Derived Output refreshes, Research Chat turns and Agent tasks. Durable operation
...
work with the typed `OperationFlightsShutdownError`, aborts Agent work with that
same restart-distinguishable reason, aborts Research work with
...
- One Agent-task id names at most one promise, controller and deadline.
- A first research stop asks the run to finish; a second aborts it.
- An Agent-task stop aborts immediately; the capability writes its terminal state.
