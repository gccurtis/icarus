...
- Reference, claim, publication input, and publication receipt objects contain
  exactly their current own fields. Missing, extra, non-enumerable, and symbolic
  fields are rejected rather than ignored as an older or wider shape.
- The declared size is a non-negative safe integer and equals both the admitted
  input and every verified read.
- Every publication carries the capability's required positive `maxBytes`
  ceiling, and storage rejects the write before publication when size exceeds it.
...
- Reads use `O_NOFOLLOW`, carry the owning operation's `AbortSignal` through
  native I/O, and never turn represented identities into arbitrary paths.
...
| --- | --- | --- |
| `put` | file | Enforce the required capability ceiling, verify and durably publish immutable admitted bytes, and retain a hash-bound recovery token until claim |
...
| `read` | file | Return hash-and-size-verified bytes, `undefined` when absent, honor cancellation, or fail on corruption |
...
├── methods/
│   ├── acquire-mutation.ts
│   ├── claim-publication.ts
│   ├── discard-publication.ts
│   ├── put.ts
│   ├── read.ts
│   ├── reconcile.ts
│   ├── release-claim.ts
...
`definition.ts` owns only instance state and bound public ports. Each method
module owns one effectful chain; shared modules contain the pure validation,
name, and filesystem primitives used by those chains. Tests follow the same
behavior boundaries so recovery, publication, and hostile I/O cases remain
