...
import { remove } from "$spreadsheets/api/remove/remove";
...
describe("remove", () => {
  ...
  it("takes the body with it, so nothing can read the workbook after", async () => {
    const { ctx, scope } = await asking();
    const id = await create(asCtx(ctx), scope, "Budget");
    const resource = { resourceType: "spreadsheet", resourceId: id } as const;
