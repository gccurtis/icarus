 ...
 * it. Nothing here is scoped: the caller resolved the resource row in its own
 * project first, and these rows are keyed by the resource pair rather than by
 * the gate.
 *
 ...
 */
export const discard = async (ctx: MutationCtx, resource: ResourceKey): Promise<void> => {
  ...
  const sets = await ctx.db
    ...
    .withIndex("by_resource_revision", (q) =>
      q.eq("resourceType", resource.resourceType).eq("resourceId", resource.resourceId)
    )
    .collect();
