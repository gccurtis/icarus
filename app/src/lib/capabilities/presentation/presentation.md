...
A presentation's body, read.

One procedure. `readPresentationBody` hands back the leader snapshot for one presentation —
a revision and a whole `PresentationBody` — or nothing, for a presentation that has never
been written to.

**Nothing here writes.** A presentation is edited in the browser and those edits are held
...
**Which project is not a parameter.** It comes from the scope, and a presentation whose
