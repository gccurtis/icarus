...
const { submitPresentationChanges } = await import(
  "$capabilities/presentation/api/submit-presentation-changes/submit-presentation-changes"
);
...
const directory = (): string => {
  const made = mkdtempSync(join(tmpdir(), "icarus-submit-presentation-"));
...
const seeded = (path: string) => {
  ...
  const resourceId = store.create("presentations", {
  ...
  const leaderId = store.create("presentationSnapshots", {
    projectId: "projects:p",
    resourceId,
    revision: 0,
    role: "leader",
...
const revisionOf = (store: StoreModel, leaderId: string): unknown => {
  const found = store.read(`presentationSnapshots.${leaderId}.revision`);
...
const expectUntouched = (store: StoreModel, ids: ReturnType<typeof seeded>): void => {
  ...
  expect(rowsIn(store, "presentationChangeSets")).toHaveLength(0);
...
const expectAdvanced = (store: StoreModel, ids: ReturnType<typeof seeded>): void => {
  ...
  expect(rowsIn(store, "presentationChangeSets")).toHaveLength(1);
  expect(rowsIn(store, "semanticSyncJobs")[0]).toMatchObject({
    ref: { kind: "presentation", id: ids.resourceId }, requestedRevision: 1, state: "queued"
  });
...
describe("submit presentation changes transaction atomicity", () => {
  it("commits revision, snapshot, metadata, and both semantic outbox lanes together", async () => {
    ...
    await expect(submitPresentationChanges(writing(ids.resourceId))).resolves.toMatchObject({
      accepted: true,
      revision: 1
    });
  ...
  it("persists a newly inserted prompt before it has configuration", async () => {
    ...
    await expect(submitPresentationChanges(insertingUnconfiguredPrompt(ids.resourceId))).resolves.toMatchObject({
      accepted: true,
      revision: 1
    });

    const snapshot = rowsIn(storeAt(path), "presentationSnapshots")[0];
  ...
  it("rolls back before decision and recovers whole after every decided boundary", async () => {
    ...
    await expect(submitPresentationChanges(writing(rollbackIds.resourceId))).rejects.toThrow(/interrupted/);
    expectUntouched(storeAt(rollbackPath), rollbackIds);

    const failpoints: StoreFailpoint[] = [
      "transaction:after-journal",
      "transaction:after-table:presentationChangeSets",
      "transaction:after-table:presentationSnapshots",
      "transaction:after-table:presentations",
    ...
    for (const failpoint of failpoints) {
      ...
      await expect(submitPresentationChanges(writing(ids.resourceId))).rejects.toThrow(/interrupted/);
      expectAdvanced(storeAt(path), ids);
    }
  });
});
