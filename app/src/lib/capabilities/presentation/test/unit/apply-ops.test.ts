...
const set = (
  target: Extract<PresentationOp, { op: "set" }>["target"],
  path: string,
  value: unknown,
...
const shapeIn = (presentation: PresentationBody, slide: number, index: number) => presentation.slides[slide].elements[index];
...
describe("set", () => {
  ...
  it("reaches a field on the presentation itself", () => {
    const next = applyOps(body(), [
      set("presentation", "theme/colors/text", "--token-ink-secondary"),
      set("presentation", "aspectRatio", "4:3")
    ]);
  ...
  it("refuses an id it cannot find", () => {
    expect(() => applyOps(body(), [set("element", "nope/frame", frame(0.5))]))
      .toThrow(/Nothing in the presentation has the id nope/);
  });
...
describe("text", () => {
  const type = (at: number, insert: string, remove = ""): PresentationOp => ({
    op: "text",
    target: "atom",
    path: "b1/atoms/b1-a",
    at,
  ...
  const blockOf = (presentation: PresentationBody) => {
    const content = presentation.slides[0].elements[0].content;
...
describe("presentation", () => {
  it("reorders slides, so a presentation can be arranged", () => {
    const next = applyOps(body(), [
