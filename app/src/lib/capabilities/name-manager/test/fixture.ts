 ...
 * Convex serializes: a plain `Error` still matches `/taken/` here and still
...
export const refusalFrom = async (call: Promise<unknown>) =>
  await call.then(
    () => undefined,
    (error: unknown) => nameManagerRefusal(error)
  );

export const variablesStored = (ctx: ReturnType<typeof fakeCtx>) =>
  [...ctx.rows.values()].filter((row) => row._table === "nameVariables");
