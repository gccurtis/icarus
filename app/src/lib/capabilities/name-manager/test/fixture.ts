 ...
 *
 * Running the real `initializeNameManager` rather than hand-written DDL means the
...
export const installDatabases = (projectIds: readonly string[] = ["project-a"]): void => {
  ...
  beforeEach(async () => {
    ...
    for (const database of stub.databases.values()) {
      // `restart identity` so `definition_order` starts from one in every test.
      // Without it, ordering assertions would still pass but the values a
      // failure message printed would depend on which tests ran before.
      await sql`truncate table name_manager_variables restart identity`.execute(database);
    }
  });
