...
import { findingsRefusal } from "$findings/errors";
import type { FindingSource } from "$findings/types/finding";
...
/** The writeup. One paragraph is enough to prove blocks round trip. */
export const body = (text: string): ContentBlock[] => [
  {
    id: "b1",
    type: "text",
    variant: "paragraph",
...
/** A page as it read on the day it was read — excerpt and all. */
export const captured = (excerpt: string): FindingSource => ({
  kind: "url",
...
export const refusalFrom = async (call: Promise<unknown>) =>
  await call.then(
    () => undefined,
    (error: unknown) => findingsRefusal(error)
  );
