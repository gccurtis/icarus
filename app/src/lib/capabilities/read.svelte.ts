 ...
 *
 * A capability hands a view a live handle rather than a value, so every one of
 * them answers with the same `current` / `error` / `refresh` shape and a panel is
 * written the same way whether the table exists yet or not. A real read arrives
 * as an `api/` beneath the capability, and the panels above it do not move.
 *
