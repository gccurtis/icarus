/**
 * The inputs the eleven functions accept.
 *
 * **Every mutation carries the revision the caller believes it is editing.** A
 * stale edit is refused rather than applied on top of someone else's commit —
 * which is the whole reason a display handle is safe to hand out: it names a
 * position *in a revision*, and the revision is checked before it is used.
 */
import type {
  AtomTextRange,
  DisplayPosition,
...
import type { AtomId, RichContentId } from "$rich-content/types/ids";
