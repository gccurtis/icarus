/**
 * Logical lines, derived from atom order.
 *
 * **Lines are never stored.** A line break is an atom, and everything between
 * two of them is one line. Storing lines as containers instead would make a mark
 * spanning a break into two marks, and every range operation would have to know
 * about the boundary.
 */
import type {
  ListItemMark,
...
export const rawLines = (content: RawContent): RawLine[] => {
  ...
  // Always pushed, so content ending in a break has a trailing empty line —
  // which is what a person sees after pressing return at the end.
  lines.push({ index: lines.length, atoms });
  return lines;
};
