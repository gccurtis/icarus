...
/**
 * Deletes a deck, and its body with it.
 *
 ...
 * so a deck whose body outlived it would stay readable and editable by anyone
 * still holding its id.
 *
 * The title is read before the row goes, because the entry has to say which deck
...
export const remove = async (
  ...
  await record(ctx, scope, {
    ...
    verb: "deleted",
    target: { type: "slideDeck", id, label: title }
  });
};
