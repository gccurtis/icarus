...
const findElement = (elements: readonly SlideElement[], path: readonly string[]): SlideElement | undefined => {
  const [id, ...rest] = path;
...
const canvasSize = (aspectRatio: string): { width: number; height: number } => {
...
const schematicCoordinate = (value: number): number => Math.round(value * 10_000) / 10_000;
...
const boundedJson = (value: unknown, maximum = 60_000): unknown => {
