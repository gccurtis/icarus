...
describe("what the overlay refuses", () => {
  ...
  it("takes an ordinary resource, and one staged in another project", () => {
    ...
    expect(isStagedResource(store, project, { kind: "document", id: asId<"documents">("documents:4") })).toBe(false);
    expect(isStagedResource(store, project, { kind: "document", id: asId<"documents">("documents:7") })).toBe(false);
  });

  it("says nothing about kinds a stage cannot be", () => {
    const store = storeOf([stage(project, "documents:1")]);
    expect(isStagedResource(store, project, { kind: "spreadsheet", id: "documents:1" } as never)).toBe(false);
    expect(isStagedResource(store, project, { kind: "externalFile::text", id: "documents:1" } as never)).toBe(false);
  });

  it("takes everything when nothing is staged at all", () => {
    const store = storeOf([]);
    expect(isStagedResource(store, project, { kind: "document", id: asId<"documents">("documents:1") })).toBe(false);
  });
});
