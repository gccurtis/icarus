...
/** The first supported authoritative text resource kinds. */
export const semanticResourceRef = (value: unknown): ResourceRef => {
  ...
  if (candidate.kind !== "document" && candidate.kind !== "slides") {
    throw new Error("A semantic resource ref kind must be 'document' or 'slides'");
  }
  if (
