...
import type { FileIngest } from "$external-files/api/ingest/ingest";
...
export const asking = async () => {
  ...
  return { ctx, scope: scopeOf(await projectNamed(ctx, "Development"), userId), userId, person };
};

/** A file arriving. Tests name what they are about and take the rest from here. */
export const arriving = (name: string, origin: FileOrigin = { kind: "upload" }): FileIngest => ({
  storageId: `_storage:${name}` as unknown as Id<"_storage">,
  name,
...
export const refusalFrom = async (call: Promise<unknown>) =>
  await call.then(
    () => undefined,
    (error: unknown) => externalFilesRefusal(error)
  );
