...
export const templateBodyNamesExternalFile = (
  body: TemplateBody,
  externalFileId: string
): boolean => {
  if (body.resource === "document") return documentBodyNamesExternalFile(body, externalFileId);
  if (body.resource === "slides") return slideDeckBodyNamesExternalFile(body, externalFileId);
  return Object.values(body.cells).some((cell) =>
    (cell.value !== undefined && formulaValueNamesExternalFile(cell.value, externalFileId)) ||
    marksNameExternalFile(cell.marks ?? [], externalFileId)
  );
};
