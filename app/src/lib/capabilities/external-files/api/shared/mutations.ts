...
export const externalFileRef = (row: TableRow<"externalFiles">) => ({
...
export const createExternalFileIn = (
  ...
  const found = unit.read(`externalFiles.${id}`);
  if (found?.kind !== "row" || found.table !== "externalFiles") {
...
export const replaceExternalFileIn = (
  ...
  if ("semanticContext" in changes && changes.semanticContext === undefined) {
