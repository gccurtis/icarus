...
/** The argument for a claim. One paragraph is enough to prove it round trips. */
export const rationale = (text: string): ContentBlock[] => [
  {
    id: "r1",
    type: "text",
    variant: "paragraph",
    atoms: [{ id: "r1a", kind: "literal", text }],
    display: text,
    marks: []
  }
];
...
export const refusalFrom = async (call: Promise<unknown>) =>
  await call.then(
    () => undefined,
    (error: unknown) => hypothesesRefusal(error)
  );
