...
Nothing here checks that the thread exists yet. `researchThreads` and
`personaThreads` are built and `agentTasks` is not, and a check covering two
kinds of three would read as a guarantee this function does not make. When all
three exist it proves the thread is in the caller's project, which is the one
check the model asks for and the only one it can.
