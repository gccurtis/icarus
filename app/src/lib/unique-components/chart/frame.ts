import type { SpreadsheetAnalytic } from "$json-store/types/data/analytic";
...
/** Placement adapter for an analytic reference stored in a spreadsheet body. */
export const frameForSpreadsheetAnalytic = (
  analytic: SpreadsheetAnalytic,
  anchorPosition: { x: number; y: number }
): ChartFrame => ({
  x: anchorPosition.x + analytic.offset.x,
  y: anchorPosition.y + analytic.offset.y,
  width: analytic.size.width,
  height: analytic.size.height
});
