...
These pages describe the code currently under [`0-platform/intelligence`](../). The older repository-wide [platform design page](../../../../../../docs/platform/intelligence.md) is useful background, but this package is authoritative where that page describes planned APIs or behavior not present in source.
...
| [`create/intelligence.ts`](../../../1-init/create/intelligence.ts) | Production construction and provider registration |
| [`loadBackendConfig.ts`](../../../0-utils/config/loadBackendConfig.ts) | Configuration parsing and default route matrix |
| [`configuration.yaml`](../../../../etc/configuration.yaml) | Committed provider, inference, reasoning, and embedding routes |
| [`startBackend.ts`](../../../1-init/startBackend.ts) | Process-level composition and injection |
