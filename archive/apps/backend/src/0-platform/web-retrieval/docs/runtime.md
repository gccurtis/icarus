...
None. There is no `WebRetrieval` class/interface in source, provider adapter, HTTP client wrapper, factory under `1-init/create`, config loader support, logger integration, test fake, or shutdown lifecycle.
...
9. production construction in `1-init` and explicit capability injection.
