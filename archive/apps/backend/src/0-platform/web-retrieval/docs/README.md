...
Web Retrieval is not implemented. [`0-platform/web-retrieval`](../) contains only [`.gitkeep`](../.gitkeep). There are no TypeScript types, runtime objects, adapters, factories, configuration fields, consumers, endpoints, jobs, tests, logs, or persistence.

The repository-wide [Web Retrieval design page](../../../../../../docs/platform/web-retrieval.md) describes an intended provider-neutral search/fetch boundary. It is a design input, not evidence that those outcomes currently exist. Research and Sources pages also refer to future consumers that are not present under `apps/backend/src/3-capabilities`.
