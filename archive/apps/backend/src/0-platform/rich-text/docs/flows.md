...
[`registerDocumentEndpoints.ts`](../../../4-job-wiring/document/registerDocumentEndpoints.ts) exposes:
...
The strict decoder in [`document/wire/commandSchemas.ts`](../../../3-capabilities/document/wire/commandSchemas.ts) accepts Document operations including `rich-text.apply`, whose payload contains `RichTextOperation[]`. [`document/domain/reducer.ts`](../../../3-capabilities/document/domain/reducer.ts) locates a text-bearing block and calls `richText.apply`; host validation calls `richText.validate` for embedded block content.
...
Document also uses Rich Text for `style.apply-inline`, where the reducer resolves a saved Document Style and calls `richText.apply` with one add-mark operation. Host inverse construction is in [`document/domain/inverses.ts`](../../../3-capabilities/document/domain/inverses.ts).
...
[`registerDocumentInternalJobs.ts`](../../../4-job-wiring/document/registerDocumentInternalJobs.ts) registers:
...
[`registerSlideEndpoints.ts`](../../../4-job-wiring/slide/registerSlideEndpoints.ts) declares serial `POST /slides/command` and concurrent `POST /slides/query` inline Jobs. Slide wire schemas strictly decode embedded Rich Content and operations. However, [`slide/index.ts`](../../../3-capabilities/slide/index.ts) references a missing `application/slideService.ts`, so startup cannot currently construct the required `SlideCapability`; these routes are not operable in the live tree.

[`slide/domain/reducer.ts`](../../../3-capabilities/slide/domain/reducer.ts) calls Rich Text for:
...
Projection source is [`document/projections/`](../../../3-capabilities/document/projections/) and [`slide/projections/`](../../../3-capabilities/slide/projections/). Host whole-range style marks ensure styling segmentation covers all authored text; persisted links are appended outside `overlayMarks` so supplementary links are not lost.
