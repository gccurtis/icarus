...
[`createLogger(config)`](../../../1-init/create/logger.ts) is the concrete runtime factory:
...
[`startBackend.ts`](../../../1-init/startBackend.ts) creates config then logger before all other runtime objects. The same instance is passed to:
...
| --- | --- | --- |
| Startup | [`startBackend.ts`](../../../1-init/startBackend.ts) | readiness, listener, recovery, startup failure |
| HTTP transport | [`registerHttpTransport.ts`](../../../2-transport/registerHttpTransport.ts) | route miss, completion, overload rejection, unexpected failure |
| Job Scheduler | [`scheduler.ts`](../../../0-utils/jobs/scheduler.ts) | enqueue/start/respond/complete, capacity, inline/deferred failure |
...
| Context | [`context.ts`](../../../3-capabilities/context/context.ts) | reads, declarations, updates, resolve/compose timing |
| Structured Data | [`structured-data.ts`](../../../3-capabilities/structured-data/structured-data.ts) | declaration/query/mutation revisions/counts |
| Formula resolver | [`formula-name-resolver.ts`](../../../1-init/create/formula-name-resolver.ts) | cache/build/issues |
| Resource registry | [`resource-reader.ts`](../../../1-init/create/resource-reader.ts) | scope resolution/read denial |
| General Files | [`generalFileService.ts`](../../../3-capabilities/general-files/application/generalFileService.ts) | upload/update/delete and Knowledge compensation |
| Connector | [`connectorService.ts`](../../../3-capabilities/connector/application/connectorService.ts) | registration/sync/read/delete/reconciliation |
| Connector scheduler | [`connectorSyncScheduler.ts`](../../../1-init/create/connectorSyncScheduler.ts) | timer lifecycle/discovery/enqueue failures |
| Derived Outputs | [`derived-outputs.ts`](../../../3-capabilities/derived-outputs/derived-outputs.ts) | definitions, refresh/tool/settlement/invalidation |
| Document | [`documentService.ts`](../../../3-capabilities/document/application/documentService.ts) | command/internal-stage/recovery failures |
| Slide composition seam | [`create/slide.ts`](../../../1-init/create/slide.ts), [`registerSlideEndpoints.ts`](../../../4-job-wiring/slide/registerSlideEndpoints.ts) | Injected logger and endpoint registration/unexpected-error events; the referenced Slide application service module is currently absent |
| Endpoint wiring | [`4-job-wiring/`](../../../4-job-wiring/) | endpoint registration and capability-specific unexpected errors |
...
[`createApp`](../../../1-init/create/app.ts) sets `Fastify({ logger: false })`, so Fastify does not create an independent stdout stream. Runtime tests guard `index.ts` and scheduler against `console.*`. This is a scoped code guard, not an automated repository-wide prohibition on every possible dependency's output.
