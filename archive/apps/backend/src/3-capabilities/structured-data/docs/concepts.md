...
[`FormulaNameResolver`](../../../1-init/create/formula-name-resolver.ts) obtains one `bindingView`, computes an entry signature, and may return a cached snapshot when IDs/revisions/names/kinds match. Otherwise it iteratively resolves entries:
