...
**Stage** — A scratch document or presentation holding a template's body, plus a row saying which template it is
...
**A template of one slide** — Save slide copies the current slide into a presentation template holding that
slide, its layout, and the theme and styles it is drawn with, and no sections. It is not a fourth kind:
afterwards it is a presentation template like any other, listed, inserted and edited the same way. `presentationOfSlide`.

## What changed about the templates you already had

| Concept | State | What is different |
| --- | --- | --- |
| Template body | unchanged | Document, presentation, or spreadsheet, as before. A template of one slide is a presentation body with one slide in it. |
...
| Provenance on resources | removed | Main's `templateId` on documents, presentations and spreadsheets is gone: a resource made from a template is a copy and knows nothing of where it came from. |
| Comments | changed | A thread cannot be started on a template's working copy; the comments capability refuses it and the presentation's comment panels say so. Comments never travel with a template. |
...
- **Findings** — one of the five resource kinds a set or a default can select, alongside documents,
  presentations, spreadsheets and research threads. An existing kind, not a new one.
...
sets are managed, and whether a presentation insert brings layouts.
