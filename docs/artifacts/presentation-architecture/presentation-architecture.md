# The presentation editor, end to end
...
procedures that move a presentation body between the browser and a JSON-file store.

## Five trees, and what may reach what

The presentation is spread across five trees on purpose, and the separation is enforced
by `pnpm lint` rather than by convention.

| Tree | Holds |
| --- | --- |
| `app-views/categories/presentation-editor/` | `content/presentation.svelte`, `context/stage.svelte`, and `procedures/` — `presentation.ts` (body shaping, the only file that names ops), `stage.ts` (geometry), `tokens.ts` (token resolution for canvas) |
| `model/client/presentation-runtimes/` | `definition.svelte.ts` (Runtime, register, `tick()`), `methods/apply · attach · sync`, `methods/flush/{coalesce,flush,rebase}`, `methods/history/` |
| `capabilities/presentation/` | `index.remote.ts`, `api/read-presentation-body/`, `api/submit-presentation-changes/` (with `apply-ops.ts`), `api/shared/leader.ts` |
| `representation/data/types/presentations/` | `body.ts` (`PresentationBody`, `Slide`, `SlideElement`, `Frame`), `op.ts` (`PresentationOp`) |
...
not touch the runtime register or `$representation` — which is why every presentation
type the view needs is re-exported through `procedures/presentation.ts`.
...
`presentation.svelte` takes no props. It reads `view.active.resourceId` for which presentation,
...
| Element frames | `stage.ts` `toPixels` / `toFrame` | Frames are fractions 0–1, so a presentation survives a change of aspect ratio. Conversion in exactly one place |
| Zoom | `TabView.zoom` | Per-tab workspace state, not presentation state. An undoable `WorkspaceOp` |
...
| Ops | `procedures/presentation.ts` | `withElementFrame` returns `{ body, ops }` together, so body and op cannot disagree |
...
One object per presentation, never per tab. It is a courier — it does not know what a
slide is.

| Member | Type | Meaning |
| --- | --- | --- |
| `body` | `PresentationBody?` | The last body the leader served |
...
```
buildClientModel()
  ├── createPresentationRuntimes(settings)   built first
  └── createWorkspaceState(…, presentations)      borrows it

view.presentationRuntime(presentationId) → presentations.attach(presentationId)
...
Inbound: **leader snapshot** → **readPresentationBody** (scope-checked) → *query
...
5. `applyOps` finds the element by id across every slide — a presentation has one flat id
   space — and replaces its frame. Nothing partial is written.
6. Two rows land together: a `presentationChangeSets` row and the advanced
   `presentationSnapshots` leader.
