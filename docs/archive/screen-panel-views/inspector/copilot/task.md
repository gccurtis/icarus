...
The *Agent* row is absent when the breadcrumb already names the persona — reached
from a Persona's Work view, the trail says *Grid Analyst › Summarise overnight
outage reports* and the row would repeat it. That is the only thing that varies by
where the task was reached from; there is one task lens and no second copy.
