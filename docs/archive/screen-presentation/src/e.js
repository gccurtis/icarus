      ...
      <div>
        ...
        ${table(["Name", "Kind", "In because", "Updated"], [
          ...
          { inspect: "resolved", cells: [
            { h: `<span class="cellname"><span class="row-i">${ic("presentation")}</span>Board Update — October</span>` }, { h: "Presentation" }, { h: "Everything in this project" }, { h: "2h", cls: "num" } ] },
          { inspect: "resolved", cells: [
  ...
  contexts: (s) => s.mode === "library" ? [
    ...
    { id: "add", label: "Add", icon: "plus", body: () =>
      pane("Add to this Context", [
        ...
        sec("By name", [
          ...
          row("Board Update — October", { icon: "presentation" }),
    ...
    { id: "contents", label: "Contents", icon: "list", body: () =>
      pane("Contents", [
        ...
        sec("Contents", [row("Q3 Resilience Memo", { icon: "doc", inspect: "resolved" }), row("Board Update — October", { icon: "presentation", inspect: "resolved" }), row("Outage Cost Model", { icon: "sheet", inspect: "resolved" })].join(""), { count: 211, flush: true })
      ].join("")) },
...
SCREENS["templates"] = {
  ...
  center: (s) => s.mode === "author" ? `
    ...
    <div class="wrap">
      ...
      <div class="chips">
        ...
        ${chip("Document")}${chip("Presentation")}${chip("Slide")}${chip("Spreadsheet")}
      </div>
      <div class="cards" style="grid-template-columns:repeat(auto-fill,minmax(210px,1fr))">
        ${[["Regulatory filing shell", "Document · Project", "4 variables", true],
           ...
           ["Board update", "Presentation · Project", "2 variables"],
  ...
  contexts: (s) => s.mode === "author" ? [
    ...
    { id: "overview", label: "Overview", icon: "info", body: () =>
      pane("Overview", [
        sec("Templates", note("A template is an ordinary body with some of it left open — the open parts are variables you fill when you use it. Authoring one is authoring a document, a presentation or a spreadsheet.")),
        sec("In this project", kv([["Templates", "5", { mono: true }], ["Documents", "1", { mono: true }], ["Presentations", "2", { mono: true }], ["Single slides", "1", { mono: true }], ["Spreadsheets", "1", { mono: true }]])),
    ...
    { id: "library", label: "Library", icon: "template", body: () =>
      pane("Library", [
        sec("Project", [row("Regulatory filing shell", { icon: "doc", sub2: "Document · 4 variables", inspect: "template-card", on: true }), row("Board update", { icon: "presentation", sub2: "Presentation · 2 variables", inspect: "template-card" }), row("Title slide", { icon: "presentation", sub2: "Slide · 1 variable", inspect: "template-card" }), row("Cost model skeleton", { icon: "sheet", sub2: "Spreadsheet", inspect: "template-card" })].join(""), { count: 5, flush: true }),
    ...
    { id: "targets", label: "Kinds", icon: "layers", body: () =>
      pane("Kinds", [
        ...
        sec("Presentation", note("A whole presentation: layouts, theme, sections.") + `<div class="btn-row" style="margin-top:6px">${btn("New", { icon: "plus", sm: true })}</div>`),
        sec("Slide", note("One slide, reusable on its own. Inserted into any presentation.") + `<div class="btn-row" style="margin-top:6px">${btn("New", { icon: "plus", sm: true })}</div>`),
    ...
    { id: "recent", label: "Recent", icon: "clock", body: () =>
      pane("Recent", [
        sec("Recently updated", [row("Regulatory filing shell", { icon: "doc", right: "2w" }), row("Board update", { icon: "presentation", right: "3w" })].join(""), { flush: true }),
  ...
  notes: {
    ...
    revised: ["Library and Authoring became buttons at the top of the panel rather than a bar across the screen.", "“Slot” became <b>variable</b> throughout, which is what they are: a resource-set variable inside a body.", "A single slide is now a template kind of its own, alongside document, presentation and spreadsheet."],
...
SCREENS["personas"] = {
  ...
  inspectors: {
    ...
    task: { crumbs: [["Personas", "persona"], ["Grid Analyst", "persona"], ["Task", null]], body: [
      ...
      sec("Produced", note("Nothing yet. A result is not a resource — promote it into a finding, document, presentation or workbook to make it retrievable."), { shut: true }),
    ...
    <div class="wrap">
      ...
      <div class="split">
        ...
        <div style="display:flex;flex-direction:column;gap:calc(var(--u)*3)">
          ...
          ${[["Ask an agent to do something", "A Persona and what to ask it", "spark", true], ["Re-run a generated block", "One prompt block in a document, presentation or workbook", "refresh"]]
