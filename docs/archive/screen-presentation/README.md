...
that a bar is in the way. The presentation is where those specifications get looked at.
...
| | |
| --- | --- |
| Current presentation | <https://claude.ai/code/artifact/22fc54e9-f6b7-47a3-8dc5-1758a025a467> |
...
subscreen — a presentation editing a slide or a layout, Research on one question or all
...
- **Pinch** on a document, a presentation or a grid to zoom it. Nothing around the work
...
src/d.js      Presentation · Spreadsheet · Research
...
```bash
cd docs/screen-presentation/src
node mkfonts.mjs            # reads app/node_modules/@fontsource, writes fonts.html
node build.mjs              # writes ../icarus-screen-presentation.html — this is what gets published
...
The presentation exists to test structure, density and hierarchy.

The one exception is the dark bezel around the frame. It is deliberately outside
the application's palette so the presentation's own controls — slide navigation, the
notes drawer — cannot be mistaken for product chrome.

## The decisions this presentation settled

The first pass drew the specifications as written. A review of it produced these
changes, which are now in the presentation and are what the specifications should be
corrected toward.
...
**Editor toolbars and headers are gone.** Document, presentation and spreadsheet
...
is a presentation, where the list of slides is the first thing you need.
...
A document, a presentation and a grid zoom. A library, a conversation and a form have no
...
selection, like a presentation editing a layout or Research showing every thread — and
...
## Where the presentation disagrees with the specs
...
the strip the navigation for a screen that already has its own. The presentation follows
...
have a library and an editor. The presentation puts that switch in the context panel's
action row rather than adding a bar across the application.

## Model gaps the presentation surfaces
...
Two the presentation itself introduced, by proposing UI ahead of the model:
...
presentation's whole value is that it looks like the thing it is arguing about.
