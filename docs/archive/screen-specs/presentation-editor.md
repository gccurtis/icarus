...
The presentation editor presents one selected slide on a freeform canvas while keeping the presentation's order, sections, layouts, theme, notes, and rich content immediately reachable. The native `PresentationBody` is authoritative; a canvas library supplies interaction and rendering mechanics.

## Center surface

### Resource header

The fixed editor header shows editable presentation title, aspect ratio, template origin, truthful save/rebase/conflict state, and live collaborator presence only from an ephemeral presence channel. Rename is a metadata edit independent of canvas changes.
...
- The selected slide is centered on a neutral pasteboard at its presentation aspect ratio.
...
| `find` | Find | Search presentation text/notes/links and navigate result to slide, element, and block. Results virtualize for large presentations. |
...
| `comments` | Comments | Presentation, current slide, selected element, and selected text filters. Open first, resolved collapsed. |
...
| --- | --- | --- |
| Presentation or nothing | Identity; aspect ratio; quick New Slide | Template provenance; attribution; handout setup |
...
Inspector ancestry should read Presentation → Slide → Element → Block → Text selection when applicable. Selecting a locked element in Layout mode identifies Layout rather than Slide as its owner.
...
- Duplicating a slide copies its complete slide fields and mints new IDs for the slide and every identified descendant in the same presentation.
...
- The presentation runtime, Fabric instance, block-editor state, buffered operations, and undo history survive tab view unmounting and release only on safe tab close.
...
- Find owns presentation-wide search.
...
- [Presentations](../data-models/general-resources/slides.md)
