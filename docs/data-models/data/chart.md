...
A chart is an identified, serializable declaration embedded in the resource
that owns it. It carries resolved data so any surface can render it and
separately records the source that refreshes those facts.
...
and `size`. Every quantitative channel must be finite.
...
The source tells an adapter where the next value set comes from. It does not
make renderers understand spreadsheet ranges or analysis queries. `data`
remains the complete resolved render input.
...
`ChartModel` has no placement fields. A general surface supplies:
...
A spreadsheet embeds placement alongside the chart:

```ts
type SpreadsheetChart = ChartModel & {
...
This lets a frame move or resize without turning placement into chart content.

## Revisions

`chart` and `chartElement` are separate operation targets. Frame changes can be
reconciled separately from editing a reference line, trend, CAGR declaration,
or text annotation. Paths use stable IDs, for example
`charts/#chart-1/elements/#element-4/label`.
