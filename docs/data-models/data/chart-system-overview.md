...
This is the review guide for the visual subcomponent of the Icarus analytic
system. Read the [analytic system overview](analytic-system-overview.md) first,
then use this document before the field-by-field [chart model reference](chart.md)
or renderer code.

The chart system has one job: preserve the meaning and identity of a
materialized chart while one `AnalyticComponent` renders it on many surfaces at
different sizes. A spreadsheet, slide, analysis page, document block, inspector
preview, export process, and AI operation should all address the same chart
parts without sharing DOM state or owning a translation layer.
...
The identified chart declaration and last resolved render data are materialized.
Coordinates, paths, ticks, regression coefficients, CAGR percentages, hover
state, and DOM nodes are derived.
...
- A custom Labels analytic channel materializes into `label`.
...
`ChartModel` has no x, y, width, or height. It lives inside an
`AnalyticComponentModel`; a surface supplies a frame around that reusable
component. A spreadsheet persists a live analytic reference plus anchor,
offset, size, and optional z-index. A slide or document supplies its own
placement wrapper without changing what the analytic or chart means.
...
The materialized chart carries both:

- `source`: provenance for the next refresh; and
- `data`: the complete resolved render input.

This is intentional materialization. It lets an `AnalyticComponent` render in a
slide, document, export, or inspector without synchronously replaying joins.
The analytic definition owns normalization, joins, filters, grouping, sorting,
and aggregation. `ChartSource` retains refresh provenance; renderers understand
neither spreadsheet ranges nor analytic plans.
...
```text
AnalyticElement                      optional surface frame interaction
└── AnalyticComponent               chart/table output switch and stale state
    └── ChartRenderer               validation, title, legend, chart type switch
        ├── PlotBars
        ├── PlotPie
...
Moving an analytic component is not a change to its data or nested visual
declaration.
...
Factories provide presentation defaults. A new bar defaults to stacked; a
clustered/grouped bar is an explicit choice. Meaningful choices such as Mekko
width mode, waterfall totals, or CAGR periods remain explicit.
...
- Executing an analytic plan and refreshing chart materialization are separate
  adapter/mutation concerns.
...
1. [Analytic system overview](analytic-system-overview.md): confirm the parent
   component and table-composition model.
2. This overview: confirm visual mental models and system principles.
3. [Chart model reference](chart.md): confirm persisted fields and per-type
   constraints.
4. `chart-model.ts`: confirm factories, reconciliation, validation, and
   derived calculations.
5. `layout.ts` and `layout-additional.ts`: confirm pure geometry.
6. Plot components: confirm mark-level events and accessibility.
7. `chart-renderer.svelte`: confirm the total type switch and legend policy.
8. `analytic-component.svelte`: confirm charts and tables share one reusable
   output boundary.
9. `analytic-element.svelte`: confirm frame interaction stays outside the
   component.
10. Unit tests and `/demo/plot`: exercise the complete system.
