...
> **Reference authority.** Intelligence is Icarus’s provider-neutral runtime for inference, reasoning, structured output, tool-using reasoning, and embeddings. The implementation lives under `apps/backend/src/0-platform/intelligence` and is constructed once by `1-init`.
