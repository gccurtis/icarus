 ...
 *
 * Keyed by project id rather than holding one database, because a stub with a
 * single database could not fail a test asserting that two projects cannot see
 * each other.
 */
export const stub = {
 ...
 *
 * That is a direct consequence of dropping the runtime object. The backend's
 * tests had to fake `NameManagerStore` because every call entered through a
 * class that bound one; there is no class now, so a test calls the procedure it
 * is testing.
 *
