...
export const record = async <T>(
  ...
  logger.debug(`name-manager.${operation}.started`, fields);

  try {
    const result = await run();
    logger.debug(`name-manager.${operation}.completed`, fields);
    return result;
  } catch (error) {
    ...
    if (error instanceof NameManagerError) {
      logger.warn(`name-manager.${operation}.rejected`, { ...fields, errorCode: error.code });
    } else {
      logger.error(`name-manager.${operation}.failed`, { ...fields, ...errorFields(error) });
    }
    throw error;
  }
};
